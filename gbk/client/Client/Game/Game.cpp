...
void ProcessCmdLine( LPTSTR lpCmdLine )
{
	...
	BOOL bFromTest = strstr( lpCmdLine, "-test" ) != 0;
	//Debug 模式
	g_bDebugMode		= strstr(lpCmdLine, "-debug")!=0;
	...
	{
		MessageBox( NULL, "请从 Launch 启动游戏。", "游戏提示", MB_OK );
		exit(0);
	}
}
...
//			::SetDlgItemText(hDlg, IDC_EDIT_USERID, szUserName);
			...
			// 名称
			::GetPrivateProfileString( "Variable", "User_NAME", "", szUserName, MAX_PATH, szSystemCfg );
			::SetDlgItemText( hDlg, IDC_EDIT_USERID, szUserName );

//			GetDlgItem( hDlg, IDC_EDIT_USERID )->SetLimitText( 12 );
//			( (CEdit*)GetDlgItem(IDC_EDIT_USERID) )->SetLimitText( 12 );
			...
			BOOL bConnectDir = FALSE;

			//直接连接服务器
			{
				bConnectDir = ::GetPrivateProfileInt("Variable", "GameServer_ConnectDirect", 0, szSystemCfg) == 1;
...
//				::EnableWindow(::GetDlgItem(hDlg, IDC_COMBO_SERVER), bConnectDir);
			...
			//服务器列表
			{
				...
				{
//					"192.168.1.62 - zyh本机服务器",
//					"192.168.1.63 - 老曾",
//					"192.168.1.75 - 王勇鸷",
//					"192.168.1.113 - 李一立",
//					"192.168.1.59 - 程序编译服务器",
					"127.0.0.1",
//					"192.168.1.133 - 游戏服务器",
				};
				...
				// 服务器端口
				::GetPrivateProfileString( "Variable", "GameServer_Port", "", szReadTemp, MAX_PATH, szSystemCfg );
				::SetDlgItemText( hDlg, IDC_EDIT_SERVER_PORT, szReadTemp );
			}
			
			{
				// 新帐号
				::SendDlgItemMessage(hDlg, IDC_COMBO_GENDER, CB_ADDSTRING, 0, (LPARAM)"女");
				::SendDlgItemMessage(hDlg, IDC_COMBO_GENDER, CB_ADDSTRING, 1, (LPARAM)"男");
				
				::GetPrivateProfileString( "Variable", "User_GENDER", "", szReadTemp, MAX_PATH, szSystemCfg );
				INT sex = atoi( szReadTemp );
				::SendDlgItemMessage(hDlg, IDC_COMBO_GENDER, CB_SETCURSEL, sex, 0);


				// GUID
				::GetPrivateProfileString( "Variable", "User_GUID", "", szReadTemp, MAX_PATH, szSystemCfg );
				::SetDlgItemText( hDlg, IDC_EDIT_USEGUID, szReadTemp );


				// 职业
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"少林派-棍武僧" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"少林派-铁罗汉" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"丐帮-棍丐帮" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"丐帮-掌丐帮" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"武当派-剑武当" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"武当派-气武当" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"唐门-刀唐" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"唐门-陷阱唐" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"逍遥派-扇逍遥" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"逍遥派-指逍遥" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"药王谷-狂药王" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"药王谷-医药王" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"桃花岛-笛桃花" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"桃花岛-音桃花" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"峨嵋派-刺峨嵋" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_PROFESSION, CB_ADDSTRING, 0, (LPARAM)"峨嵋派-辅峨嵋" );

				::GetPrivateProfileString( "Variable", "User_MENPAI", "", szReadTemp, MAX_PATH, szSystemCfg );
				INT menpai = atoi( szReadTemp );
				::SendDlgItemMessage(hDlg, IDC_COMBO_PROFESSION, CB_SETCURSEL, menpai, 0);



				// 国家	
				::SendDlgItemMessage( hDlg, IDC_COMBO_COUNTRY, CB_ADDSTRING, 0, (LPARAM)"新月汗国" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_COUNTRY, CB_ADDSTRING, 0, (LPARAM)"金帐汗国" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_COUNTRY, CB_ADDSTRING, 0, (LPARAM)"西域汗国" );
				::SendDlgItemMessage( hDlg, IDC_COMBO_COUNTRY, CB_ADDSTRING, 0, (LPARAM)"大漠汗国" );

				::GetPrivateProfileString( "Variable", "User_COUNTRY", "", szReadTemp, MAX_PATH, szSystemCfg );
				INT country = atoi( szReadTemp );
				::SendDlgItemMessage(hDlg, IDC_COMBO_COUNTRY, CB_SETCURSEL, country, 0);
			}
			
			if( !bConnectDir )
			{
				::EnableWindow( ::GetDlgItem(hDlg, IDC_EDIT_USERID),  FALSE );
				::EnableWindow( ::GetDlgItem(hDlg, IDC_COMBO_GENDER), FALSE );
				::EnableWindow( ::GetDlgItem(hDlg, IDC_EDIT_USEGUID), FALSE );
				::EnableWindow( ::GetDlgItem(hDlg, IDC_COMBO_PROFESSION), FALSE );
				::EnableWindow( ::GetDlgItem(hDlg, IDC_COMBO_COUNTRY), FALSE );
			}
...
//						::EnableWindow(::GetDlgItem(hDlg, IDC_COMBO_SERVER), TRUE);
						::EnableWindow( ::GetDlgItem(hDlg, IDC_EDIT_USERID),  TRUE );
						::EnableWindow( ::GetDlgItem(hDlg, IDC_COMBO_GENDER), TRUE );
						::EnableWindow( ::GetDlgItem(hDlg, IDC_EDIT_USEGUID), TRUE );
						::EnableWindow( ::GetDlgItem(hDlg, IDC_COMBO_PROFESSION), TRUE );
			...
			case IDOK:
				{
					...
					{	
						...
						//取得新ID
						{
							...
							::GetPrivateProfileString( "Variable", "User_NAME", "", szNameTemp, MAX_PATH, szSystemCfg );

							GetDlgItemText(hDlg, IDC_EDIT_USERID, szTemp, MAX_PATH);	// 用户名

							// 通过名称来判断是否是新的角色
							if( 0 != strcmp( szNameTemp, szTemp ))
							{
								...
								//保存ID
//								::SHSetValue(HKEY_LOCAL_MACHINE, "SOFTWARE\\KYLIN", "UserID", REG_SZ, szTemp, (UINT)strlen(szTemp));
								...
								// 职业
								{
									INT nNewPlayer_Menpai = (INT)::SendDlgItemMessage(hDlg, IDC_COMBO_PROFESSION, CB_GETCURSEL, 0, 0); 
									_snprintf(szTemp, MAX_PATH, "%u", nNewPlayer_Menpai);
									::WritePrivateProfileString( "Variable", "User_MENPAI", szTemp, szSystemCfg );
								}

								// 国家
								{
									INT nNewPlayer_Country = (INT)::SendDlgItemMessage( hDlg, IDC_COMBO_COUNTRY, CB_GETCURSEL, 0, 0 ); 
									_snprintf( szTemp, MAX_PATH, "%u", nNewPlayer_Country );
					...
					//服务器
					{
						...
						// 服务器端口
						GetDlgItemText(hDlg, IDC_EDIT_SERVER_PORT, szTemp, MAX_PATH);
						::WritePrivateProfileString( "Variable", "GameServer_Port", szTemp, szSystemCfg );
					}
					...
					{
						UINT dwDontShow = 1;
						SHSetValue(HKEY_LOCAL_MACHINE, "SOFTWARE\\KYLIN", "DontShow", REG_DWORD, &dwDontShow, sizeof(UINT));
					}
...
VOID Debug_ChooseUserSetting(VOID)
{
	...
	if( ERROR_SUCCESS == ::SHGetValue(HKEY_LOCAL_MACHINE, "SOFTWARE\\KYLIN", "DontShow", &dwType, &dwDontShow, &dwSize) &&
		dwDontShow == 1 )
	{
		return;
	}
