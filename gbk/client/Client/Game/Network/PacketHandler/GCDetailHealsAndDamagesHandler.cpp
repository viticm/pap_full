	...
	__ENTER_FUNCTION
		if(CGameProcedure::GetActiveProcedure() == (CGameProcedure*)CGameProcedure::s_pProcMain)
		{
			...
			if ( pObj != NULL )
			{
				...
				infoDamage.m_bBeatBack				= pPacket->IsBeatBack();
				infoDamage.m_bBeatFly				= pPacket->IsBeatFly();
				infoDamage.m_BeatBackPos			= pPacket->GetBeatBackPos();
				infoDamage.m_CloseToPos				= pPacket->GetCloseToPos();
				infoDamage.m_fBeatFlyDist			= pPacket->GetBeatFlyDistance();
				infoDamage.m_fAttackerAngle			= pPacket->GetAttackerAngle();
				infoDamage.m_bCloseTo				= pPacket->IsCloseTo();
