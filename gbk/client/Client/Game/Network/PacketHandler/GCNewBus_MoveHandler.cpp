...
uint GCNewBus_MoveHandler :: Execute(GCNewBus_Move* pPacket, Player* pPlayer) 
{
	if(CGameProcedure::GetActiveProcedure() == (CGameProcedure*)CGameProcedure::s_pProcMain)
	{
		...
		{
			KLThrow("ERROR POSITION @ GCNewBus_MoveHandler");
		}
...

		{
			...
			cmdTemp.m_afParam[1]	= pPacket->GetTargetPosHeight();
			cmdTemp.m_afParam[2]	= pPacket->GetTargetPos()->m_fZ;
			pBus->PushCommand(&cmdTemp);
		}

		pBus->PushDebugString("GCNewBus_Move");
	}
