...
VOID CWorldManager::ChangeScene(INT idTargetScene, INT idTargetSceneResID, const fVector2& fvPos, INT nDirection)
{
	...
	DBC_DEFINEHANDLE(pMappicDBC,   DBC_MAPPIC_DATA);	// 场景跳转中图片表
	DBC_DEFINEHANDLE(pChatHelpDBC, DBC_CHATHELP_DATA);	// 场景跳转中文字提示表
...
VOID WINAPI CWorldManager::_OnVariableChangedEvent(const EVENT* pEvent, UINT dwOwnerData)
{
	...
	if(pEvent && pEvent->pEventDef && pEvent->pEventDef->idEvent == GE_VARIABLE_CHANGED)
	{
		...
		else if ( strVariable == CSoundSystemFMod::szAllSoundDisable ) // 关闭/启动全部音效		20100714 BLL
		{
			((CScene*)GetMe()->GetActiveScene())->ControlBkgSound( !bEnable );
			((CScene*)GetMe()->GetActiveScene())->ControlEnvSound( !bEnable );
		}
...
{
	//		m_PlayerlistObj.clear();			// 别的玩家
	//		m_PKlistObj.clear();				// PK对手
	//		m_PKKilllistObj.clear();			// PK红名的玩家

	if( m_bAnimyNeedFresh )
	{
		m_AnimylistObj.clear();		// 敌人列表
		m_FriendlistObj.clear();	// 队友列表

		m_FamilylistObj.clear();			// 家族列表
		m_GuildlistObj.clear();				// 帮会列表
		m_HomelandPlayerlistObj.clear();	// 本国别的玩家
		m_NoHomelandPlayerlistObj.clear();	// 非本国别的玩家

		m_ExpObjlistObj.clear();	// 生长点列表
		m_ScenePoslistObj.clear();	// 场景传送点列表

		m_ExpNPClistObj.clear();	// NPC列表
		m_MissCompleteNpcObj.clear();	// 任务NPC列表(完成)
		m_MissNoCompleteNpcObj.clear();	// 任务NPC列表(未完成)
		m_MissionNpcObj.clear();	    // 任务NPC列表(可接)

		m_PetlistObj.clear();		// 宠物点列表

		MAP_POS_DEFINE data;

		m_pActiveManager->update();
		m_pActiveManager->GetActiveList( GetActiveSceneID(), &m_ActivelistObj );	// 激活点列表
		m_pActiveManager->GetFlashList(  GetActiveSceneID(), &m_FlashlistObj );		// 闪光点列表
...
//								m_PlayerlistObj.push_back( data );
									...
									if( INVALID_ID != nMyGuild )
									{
										...
										if( INVALID_ID != nGuild )
										{
											...
											if( nMyGuild == nGuild )
											{
												...
												if( pMember )
													nMyFamily = pMember->m_iFamilyID;	// 所属家族

												pMember = CDataPool::GetMe()->Guild_GetMemInfoByIndex( nGuild );
												if( pMember )
													nFamily = pMember->m_iFamilyID;		// 所属家族
								...
								// 判断国家
								{
									...
									if( nMyCountry == nCountry )
										m_HomelandPlayerlistObj.push_back( data );		// 本国别的玩家
									// 非本国
									else
										m_NoHomelandPlayerlistObj.push_back( data );	// 非本国别的玩家
								}
					...
					else if( g_theKernel.IsKindOf( pCurObj->GetClass(), GETCLASS(CTripperObject_Resource) ) )
					{
						...
						/*_snprintf( szInfo, "ExpObj %s所处位置 x:%f, y%f\n", data.name, data.pos.x, data.pos.y );
...
{
	//	case :	return &m_PKKilllistObj;			// pk红名
	//	case :	return &m_PKlistObj;				// pk对象
	//return &m_PlayerlistObj; // OtherPlayer 别的玩家

	switch( nType )
	{
	case MINIMAP_TYPE_ANIMY:  return &m_AnimylistObj;		// Animy 敌人
	case MINIMAP_TYPE_TEAM:	  return &m_FriendlistObj;		// Teamate 队友

	case MINIMAP_TYPE_FAMILY:	   return &m_FamilylistObj;				// 家族列表	
	case MINIMAP_TYPE_GUILD:	   return &m_GuildlistObj;				// 帮会列表	
	case MINIMAP_TYPE_HOMELAND:	   return &m_HomelandPlayerlistObj;	    // 本国别的玩家			
	case MINIMAP_TYPE_NOHOMELAND:  return &m_NoHomelandPlayerlistObj;	// 非本国别的玩家

	case MINIMAP_TYPE_EXPOBJ:	return &m_ExpObjlistObj;		// ExpObj 生长点
	case MINIMAP_TYPE_ACTIVE:	return &m_ActivelistObj;		// 方向 激活方向点
	case MINIMAP_TYPE_SCENEPOS: return &m_ScenePoslistObj;		// 场景跳转点
	case MINIMAP_TYPE_FLASH:	return &m_FlashlistObj;			// 闪光点

	case MINIMAP_TYPE_EXPNPC:	    return &m_ExpNPClistObj;		// ExpNPC 普通npc
	case MINIMAP_TYPE_MISCOMNPC:	return &m_MissCompleteNpcObj;	// 任务NPC列表(完成)
	case MINIMAP_TYPE_MISNOCOMNPC:	return &m_MissNoCompleteNpcObj;	// 任务NPC列表(未完成)
	case MINIMAP_TYPE_ACCEPTNPC:	return &m_MissionNpcObj;	    // 任务NPC列表(可接)	

	case MINIMAP_TYPE_PET:return &m_PetlistObj;				// 宠物点
