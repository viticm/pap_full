...
//============================================================================================
FLOAT CCamera_Scene::STATIC_PITCH	= -38.5f * KLU_PI / 180.0f;		// 固定视角
...
FLOAT CCamera_Scene::MIN_ANGLE		= 0.1f * KLU_PI / 180.0f;		// 角度最小计算误差
FLOAT CCamera_Scene::STEP_ANGLE		= 1.0f * KLU_PI / 180.0f;		// 角度最小调动范围
...
//	fvGfxLookatPos.x -= m_fOffset;
	...
	m_fPitch = _GetValidPitch(
						CEngineInterface::GetMe()->GetFairySystem()->getTerrainData(),	// 地形数据
						fvGfxLookatPos,													// 关注点
						m_fDistance,													// 距离
						m_fDirection,													// 旋转角度
						m_fPitch,														// 目前的Pitch(尽量接近此值)
						m_fPitchApproch,	// STATIC_PITCH,							// 要趋近的Pitch
						m_pOgreCamera->getNearClipDistance()
						);

	//& 根据这个值来判断， STATIC_PITCH只用来第一次时赋值
//	m_fPitchApproch;
