...
KL_IMPLEMENT_DYNAMIC(CObject_Special, GETCLASS(CObject_Effect));
...
//	SetLogicCount( nLogicCount );
	SetActionLogicCount( nLogicCount );	// 可能存在问题， 看实际效果
	...
	if ( GetSpecialObjData() != NULL && GetSpecialObjData()->m_nBulletID != INVALID_ID )
	{
		...
		initBullet.m_nSendID			= GetServerID();
		initBullet.m_nSendLogicCount	= GetActionLogicCount();	// GetLogicCount()
		...
		{
			initBullet.m_nTargetID	= paTargetIDs[i];
