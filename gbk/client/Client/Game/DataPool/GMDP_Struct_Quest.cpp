...
//	strcpy(*pDest,tokens[j]);
	...
	if(!pCur)
		return KLThrow("策划写错了这个脚本:%s", pSrc);
...
void parseDemandKill(CHAR** pDest,CHAR* pSrc,INT j)
{
	...
	int npcId = 0;
	int npcCount = 0;
	while(pCur++)  //x960001_g_DemandKill ={{id=9123,num=2}}
	{
		if(*pCur == '{') 
		{
			digit = 1;
...
void parseItemBonus(CHAR** pDest,CHAR* pSrc,INT j)
{
	...
	while(pCur++)
	{
		...
		else if(digit == 3 && isdigit(*pCur))
		{
			...
			if(!pItem)
			{
				KLThrow("非法的奖励物品ID:%d", rewardItem->m_ItemBonus.m_uItemID);
			}
