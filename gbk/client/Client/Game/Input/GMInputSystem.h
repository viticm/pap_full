...
//输入管理器
class CInputSystem : public tInputSystem
{
public:
...
public:
	...
	//判断是否某个系统键按下
	virtual BOOL IsCtrlDown() { return IsKeyDown(KC_LCONTROL) || IsKeyDown(KC_RCONTROL);}
	virtual BOOL IsShiftDown() { return IsKeyDown(KC_LSHIFT) || IsKeyDown(KC_RSHIFT);}
	virtual BOOL IsAltDown() { return IsKeyDown(KC_LMENU) || IsKeyDown(KC_RMENU);}
...
public:
	...
	virtual UINT	MouseGetFlagOld(VOID)			{ return m_dwMouseFlagOld; }
			BOOL	MouseIsInClient(VOID) const		{ return m_bMouseInClient; }
...
public:
	...
	static CInputSystem* GetMe(VOID) { return s_pMe; }

protected:
	static CInputSystem*		s_pMe;
...
protected:
	...
	KL_DECLARE_DYNAMIC(CInputSystem);
