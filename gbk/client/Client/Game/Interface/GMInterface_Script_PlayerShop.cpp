...
namespace SCRIPT_SANDBOX
{
	...
	INT PlayerShop::CreateShop(LuaPlus::LuaState* state)
	{
		...
		if(strName == "")
		{
			...
			//CEventSystem::GetMe()->PushEvent(GE_NEW_DEBUGMESSAGE,"对不起，需要你给店铺起个名字。");
			return 0;
		}
		if(strName.size()>12)
		{
			...
			CEventSystem::GetMe()->PushEvent(GE_NEW_DEBUGMESSAGE, strTemp.c_str());
			//CEventSystem::GetMe()->PushEvent(GE_NEW_DEBUGMESSAGE,"对不起，你的名字过长。");
			return 0;
		}
	...
	INT PlayerShop::ApplyMoney(LuaPlus::LuaState* state)
	{
		...
		else if(str == "immitbase_ok")
		{
			...
			if( (INT) (nMoney * (1+SHOP_TAX)) > nSelfMoney)
			{
				...
				CEventSystem::GetMe()->PushEvent(GE_NEW_DEBUGMESSAGE, strTemp.c_str());
				//CEventSystem::GetMe()->PushEvent(GE_NEW_DEBUGMESSAGE,"你身上的钱不够");
				return 0;
			}
	...
	INT PlayerShop::DownSale(LuaPlus::LuaState* state)
	{
		...
		if( str == "item" )
		{
			if(nConIndex == -1 || nPosition == -1)
			{
				...
				CEventSystem::GetMe()->PushEvent(GE_NEW_DEBUGMESSAGE, strTemp.c_str());
				//CEventSystem::GetMe()->PushEvent(GE_NEW_DEBUGMESSAGE,"请先选择一个物品");
				return 0;
			}
		...
		else
		{
			...
			//	CEventSystem::GetMe()->PushEvent(GE_NEW_DEBUGMESSAGE, strTemp.c_str());
			//	//CEventSystem::GetMe()->PushEvent(GE_NEW_DEBUGMESSAGE,"请先选择一个宠物");
			//	return 0;
			//}
			//SDATA_PET* pPet = CDataPool::GetMe()->PlayerShop_GetPet(TRUE, nConIndex, nPosition);
			//if(!pPet)
