...
namespace SCRIPT_SANDBOX
{
	...
	INT StallSale::ReferItemPrice(LuaPlus::LuaState* state)
	{
		...
		if(nMoney==0)
		{
			...
			//CEventSystem::GetMe()->PushEvent(GE_NEW_DEBUGMESSAGE, "上架失败，上架商品价格不能为0");
			return 0;
		}
	...
	// add by gh 2010/04/15 （等待服务器确认之后再关闭对话框）
	...
	// 设置保存状态决定是否保存摊位信息 需要完善add by gh 2010/04/15
	...
	{
		//关闭StallMessage
		CEventSystem::GetMe()->PushEvent(GE_CLOSE_STALL_MESSAGE); // 关闭摊位留言界面
		//关闭InputMoney
		CEventSystem::GetMe()->PushEvent(GE_CLOSE_INPUT_MONEY);	  // 关闭金钱输入界面
		CEventSystem::GetMe()->PushEvent(GE_CLOSE_PET_LIST);	  // 关闭宠物列表	
		return 0;
	}
	//获得摊位名
	...
	INT StallSale::PetUpStall(LuaPlus::LuaState* state)
	{
		...
		//	CEventSystem::GetMe()->PushEvent(GE_NEW_DEBUGMESSAGE, strTemp.c_str());
		//	//CEventSystem::GetMe()->PushEvent(GE_NEW_DEBUGMESSAGE,"上架失败，上架价格不能为0");
		//	return 0;
		//}
	...
	INT StallSale::PetReprice(LuaPlus::LuaState* state)
	{
		...
		//	CEventSystem::GetMe()->PushEvent(GE_NEW_DEBUGMESSAGE, strTemp.c_str());
		//	//CEventSystem::GetMe()->PushEvent(GE_NEW_DEBUGMESSAGE,"改价失败，上架价格不能为0");
		//	return 0;
		//}
