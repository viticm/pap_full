...
/*************************************************************************
	This is based somewhat on MapIterator in the CEGUI library (www.cegui.org.uk)
*************************************************************************/
#pragma once
...
public:
	...
	*/
	IteratorBrowser(typename T::iterator start_iter, typename T::iterator end_iter) :
	...
	/*!
	\brief
		IteratorBrowser destructor
	*/
	~IteratorBrowser(void)
	{
	}
	...
	/*!
	\brief
		IteratorBrowser copy constructor
	*/
	IteratorBrowser(const IteratorBrowser<T>& org) :
	...
	/*!
	\brief
		IteratorBrowser assignment operator
	*/
	IteratorBrowser<T>&	operator=(const IteratorBrowser<T>& rhs)
	{
	...
	*/
	...
	\note
		The iterator is checked, and this call will always succeed, so do not rely on some exception to exit a loop.
	*/
	IteratorBrowser<T>	operator++(int)
	{
		IteratorBrowser<T> tmp = *this;
		++*this;
	...
	*/
	bool	operator==(const IteratorBrowser<T>& rhs) const
	{
...
private:
	...
	*************************************************************************/
	...
	/*************************************************************************
		Implementation Data
	*************************************************************************/
	typename T::iterator	d_currIter;		//!< 'real' iterator describing the current position within the collection.
	typename T::iterator	d_startIter;	//!< 'real' iterator describing the start position within the collection (or what we were told was the start).
	typename T::iterator	d_endIter;		//!< 'real' iterator describing the end position within the collection (or what we were told was the end).
};
