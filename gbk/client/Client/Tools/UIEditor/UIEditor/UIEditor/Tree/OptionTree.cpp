...
{
	// Delete normal font
	if (m_fNormalFont->GetSafeHandle() != NULL)
	{
	...
	// Delete bold font
	if (m_fBoldFont->GetSafeHandle() != NULL)
	{
...
LRESULT COptionTree::SendNotify(UINT uNotifyCode, COptionTreeItem* otiItem)
{
	...
	if (lpnm != NULL)
	{
		uID = OPTION_TREE_NOTIFY_ID/*(UINT)::GetMenu(GetSafeHwnd())*/;
...
void COptionTree::UpdatedItems()
{
	...
	if (!IsWindow(GetSafeHwnd()) || m_bUpdateFlag)
	{
		return;
	}
...
void COptionTree::OnPaint() 
{
	...
	// End paint
	EndPaint(&ps);
	printf("%d.COptionTree::OnPaint end\n",i);
}
