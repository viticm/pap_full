...
void CPatchWorkerDlg::OnBnClickedButtonFoldOld()
{
	...
	if(!m_fileCollecterOld.CollectPath(szOldPath, szVersionFile, strVersion))
	{
...
void CPatchWorkerDlg::OnBnClickedButtonFoldNew()
{
	...
	BOOL bAlreadyDefined = m_fileCollecterNew.CollectPath(szNewPath, szVersionFile, strVersion);
	...
	if(bAlreadyDefined)
	{
		...
		{
			//生成新版本文件
			m_fileCollecterNew.OutputVersionFile(szVersionFile, dlg.GetVersion());
		}
	...
	else
	{
		...
		m_fileCollecterNew.ClearWorlk();
		bAlreadyDefined = m_fileCollecterNew.CollectPath(szNewPath, szVersionFile, strVersion);
...
void CPatchWorkerDlg::GeneratePatchThread(CPatchMaker* pMaker)
{
	...
	bool bret = pMaker->WritePatchFile();
...
void CPatchWorkerDlg::OnBnClickedButtonGen()
{
	...
	BOOL bTestVersion= ::GetPrivateProfileInt("VersionType","Version",0,"./PatchWorker.ini");

	//Compare
	CPatchMaker maker(m_fileCollecterOld, m_fileCollecterNew, szPatchPath);
	maker.GeneratePatchData( (LPCSTR)m_strFilter );
	maker.SetTestVersionFlag(bTestVersion);
