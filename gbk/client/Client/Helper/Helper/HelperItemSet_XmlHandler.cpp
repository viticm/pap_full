...
void CHelperItemSet_xmlHandler::elementStart(const String& element, const CHelperXMLAttributes& attributes)
{
	...
	{
		// get name of item.
        WString itemName(attributes.getValueAsWString(ItemNameAttribute));

		// get anchor for new item
        String itemAnchor(attributes.getValueAsString(ItemAnchorAttribute));
		
		// get url for new item
        WString itemWUrl(attributes.getValueAsWString(ItemUrlAttribute));
		...
		if(!itemUrl.empty())
		{
			...
			String strFullURL;
			AfxParseURL(itemWUrl.c_str(), dwServiceType, strServer, strObject, nPort);
