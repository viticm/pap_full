...
RenderSystemConfigDialog::OnInitDialog(wxInitDialogEvent &e)
{
	...
	wxChoice* RenderCtrl = (wxChoice*) FindWindow(ID_RENDERSYSTSELECT);
    ...
    for (Ogre::RenderSystemList::const_iterator it = renderers->begin(); it != renderers->end(); ++it)
    {
		...
		RenderCtrl->Insert(AS_STRING(rs->getName()),cnt++);
		 RenderCtrl->SetStringSelection( AS_STRING(rs->getName()) );
...
RenderSystemConfigDialog::TransferDataFromWindow(void)
{
    if (!wxDialog::TransferDataFromWindow())
	 ...
	 wxChoice* nameCtrl = (wxChoice*) FindWindow(ID_RENDERSYSTSELECT);
	int selected = nameCtrl->GetSelection();  // int selectedPage = mPropertyGridManager->GetSelectedPage();
    const Ogre::RenderSystemList* renderers = Ogre::Root::getSingleton().getAvailableRenderers();
    Ogre::RenderSystem* rs = (*renderers)[selected];
...
const char* RenderSystemConfigDialog::getConfigOptionsItem(int page,char* key)
{
	...
	for (Ogre::ConfigOptionMap::const_iterator itopt = opts.begin(); itopt != opts.end(); ++itopt)
	{
		const Ogre::ConfigOption& option = itopt->second;
		if(option.name == key)
			return option.currentValue.c_str();
	}
