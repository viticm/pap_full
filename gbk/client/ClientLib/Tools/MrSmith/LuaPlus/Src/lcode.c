/*
** $Id: lcode.c,v 1.1 2006/03/17 08:47:34 chaojin Exp $
...
static int nil_constant (FuncState *fs) {
  ...
  TObject k, v;
  setnilvalue2n(&v);
  sethvalue2n(&k, fs->h);  /* cannot use nil as key; instead use table itself */
  ret = addk(fs, &k, &v);
  setnilvalue(&k);
  setnilvalue(&v);
  return ret;
...
int luaK_code (FuncState *fs, Instruction i, int line) {
  ...
  luaM_setname(fs->L, "Lua_lineinfo");
  luaM_growvector(fs->L, f->code, fs->pc, f->sizecode, Instruction,
                  MAX_INT, "code size overflow");
  luaM_setname(fs->L, NULL);
  f->code[fs->pc] = i;
