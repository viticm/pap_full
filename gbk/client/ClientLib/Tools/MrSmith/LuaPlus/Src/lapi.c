/*
** $Id: lapi.c,v 1.1 2006/03/17 08:47:34 chaojin Exp $
** Lua API
...
LUA_API void lua_xmove (lua_State *from, lua_State *to, int n) {
  ...
  while (n--) {
	from->top--;
    setobj2s(to->top, from->top);
	setnilvalue(from->top);
    api_incr_top(to);
  }
  lua_unlock(to);
#else !LUA_REFCOUNT
...
LUA_API void lua_settop (lua_State *L, int idx) {
  ...
  if (idx >= 0) {
	...
	lua_assert(L->top == origTop + (origIdx + 1));
#else !LUA_REFCOUNT
    api_check(L, -(idx+1) <= (L->top - L->base));
...
LUA_API int lua_isstring (lua_State *L, int idx) {
  ...
  return (t == LUA_TSTRING || t == LUA_TNUMBER || t == LUA_TWSTRING);
}
