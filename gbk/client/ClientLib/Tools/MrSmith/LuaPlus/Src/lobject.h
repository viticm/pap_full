/*
** $Id: lobject.h,v 1.1 2006/03/17 08:47:35 chaojin Exp $
...
#define __AddRef(o) { TObject* i_o2 = (o); if(iscollectable(i_o2))	\
		{ \
			gcvalue(i_o2)->gch.ref++; \
		}  }

#define __AddRefDirect(o) { GCObject* i_o2 = (GCObject*)(o); \
			i_o2->gch.ref++; \
		}

#define __Release(L,o) { TObject* i_o2 = (o); if(iscollectable(i_o2) && ((--gcvalue(i_o2)->gch.ref)<=0))	\
		{	\
			ReleaseObject(L,gcvalue(i_o2));	\
		}	}

#define __ReleaseDirect(L,o) { GCObject* i_o2 = (GCObject*)(o); if((--i_o2->gch.ref)<=0)	\
		{	\
			ReleaseObject(L,i_o2);	\
		}	}

#define __MakeBak(v) TObject bak = *v;

extern void ReleaseObject(lua_State *L, GCObject* header);
...
#define __AddRef(o)
#define __AddRefDirect(o)
#define __Release(L,o)
#define __ReleaseDirect(L,o)
#define __MakeBak(v)
...
#define setnvalue(obj,x) \
  { TObject *i_o=(obj); __Release(L,i_o); i_o->tt=LUA_TNUMBER; i_o->value.n=(x); }

#define chgnvalue(obj,x) \
	check_exp(ttype(obj)==LUA_TNUMBER, (obj)->value.n=(x))

#define setpvalue(obj,x) \
  { TObject *i_o=(obj); __Release(L,i_o); i_o->tt=LUA_TLIGHTUSERDATA; i_o->value.p=(x); }

#define setbvalue(obj,x) \
  { TObject *i_o=(obj); __Release(L,i_o); i_o->tt=LUA_TBOOLEAN; i_o->value.b=(x); }

#define setsvalue(obj,x) \
  { TObject *i_o=(obj); __MakeBak(i_o); i_o->tt=LUA_TSTRING; \
    i_o->value.gc=cast(GCObject *, (x)); __AddRefDirect(i_o->value.gc); __Release(L,&bak); \
    lua_assert(i_o->value.gc->gch.tt == LUA_TSTRING); }

#define setuvalue(obj,x) \
  { TObject *i_o=(obj); __MakeBak(i_o); i_o->tt=LUA_TUSERDATA; \
    i_o->value.gc=cast(GCObject *, (x)); __AddRefDirect(i_o->value.gc); __Release(L,&bak); \
    lua_assert(i_o->value.gc->gch.tt == LUA_TUSERDATA); }

#define setthvalue(obj,x) \
  { TObject *i_o=(obj); __MakeBak(i_o); i_o->tt=LUA_TTHREAD; \
    i_o->value.gc=cast(GCObject *, (x)); __AddRefDirect(i_o->value.gc); __Release(L,&bak); \
    lua_assert(i_o->value.gc->gch.tt == LUA_TTHREAD); }

#define setclvalue(obj,x) \
  { TObject *i_o=(obj); __MakeBak(i_o); i_o->tt=LUA_TFUNCTION; \
    i_o->value.gc=cast(GCObject *, (x)); __AddRefDirect(i_o->value.gc); __Release(L,&bak); \
    lua_assert(i_o->value.gc->gch.tt == LUA_TFUNCTION); }

#define sethvalue(obj,x) \
  { TObject *i_o=(obj); __MakeBak(i_o); i_o->tt=LUA_TTABLE; \
    i_o->value.gc=cast(GCObject *, (x)); __AddRefDirect(i_o->value.gc); __Release(L,&bak); \
    lua_assert(i_o->value.gc->gch.tt == LUA_TTABLE); }

#define setnilvalue(obj) { TObject *i_o=(obj); __Release(L,i_o); i_o->tt=LUA_TNIL; }

#define setwsvalue(obj,x) \
  { TObject *i_o=(obj); __MakeBak(i_o); i_o->tt=LUA_TWSTRING; \
    i_o->value.gc=cast(GCObject *, (x)); __AddRefDirect(i_o->value.gc); __Release(L,&bak); \
...
#define setobj(obj1,obj2) \
  { TObject *o2=(TObject*)(obj2); TObject *o1=(obj1); \
    checkconsistency(o2); __AddRef(o2); __Release(L,o1); \
...
#define setobj2n(obj1,obj2)  \
  { const TObject *o2=(obj2); TObject *o1=(obj1); \
    checkconsistency(o2); \
    o1->tt=o2->tt; o1->value = o2->value; __AddRef(o1); }
...
#define setsvalue2n(obj,x) \
  { TObject *i_o=(obj); i_o->tt=LUA_TSTRING; \
    i_o->value.gc=cast(GCObject *, (x)); __AddRefDirect(i_o->value.gc); \
    lua_assert(i_o->value.gc->gch.tt == LUA_TSTRING); }
