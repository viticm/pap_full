/*
** $Id: lmem.c,v 1.1 2006/03/17 08:47:35 chaojin Exp $
...
void *luaM_realloc (lua_State *L, void *block, lu_mem oldsize, lu_mem size) {
  ...
  if (size == 0) {
    if (block != NULL) {
      (*G(L)->freeFunc)(block, oldsize, G(L)->memData);
      luaM_setname(L, NULL);
      block = NULL;
    }
...
#ifdef _DEBUG
    block = (*G(L)->reallocFunc)(block, oldsize, size, G(L)->memData, L->allocName, L->allocFlags);
#else !_DEBUG
    block = (*G(L)->reallocFunc)(block, oldsize, size, G(L)->memData, NULL, L->allocFlags);
#endif _DEBUG
    luaM_setname(L, NULL);
    if (block == NULL) {
      if (L)
