/*
** $Id: ltable.c,v 1.1 2006/03/17 08:47:36 chaojin Exp $
** Lua tables (hash)
...
static void setarrayvector (lua_State *L, Table *t, int size) {
  int i;
  luaM_setname(L, "Lua_arrayvector");
  luaM_reallocvector(L, t->array, t->sizearray, size, TObject);
  luaM_setname(L, NULL);  // Just in case it didn't get reset.
...
static void setnodevector (lua_State *L, Table *t, int lsize) {
  ...
  if (lsize == 0) {  /* no elements to hash part? */
    ...
    luaM_setname(L, "Lua_nodevector");
    t->node = luaM_newvector(L, size, Node);
    luaM_setname(L, NULL);  // Just in case it didn't get reset.
    for (i=0; i<size; i++) {
      t->node[i].next = NULL;
      setnilvalue2n(gkey(gnode(t, i)));
      setnilvalue2n(gval(gnode(t, i)));
    }
  }
...
Table *luaH_new (lua_State *L, int narray, int lnhash) {
  Table *t;
  luaM_setnameif(L, "Lua_Table");
  t = luaM_new(L, Table);
#if LUA_REFCOUNT
  t->ref = 0;
#endif LUA_REFCOUNT
  luaC_link(L, valtogco(t), LUA_TTABLE);
  t->metatable = hvalue(defaultmetatypes(L, LUA_TTABLE));
#if LUA_REFCOUNT
  if (t->metatable)
	__AddRefDirect(t->metatable);
#endif LUA_REFCOUNT
...
*/
static const TObject *luaH_getkeyany (Table *t, const TObject *key) {
  if (ttisnil(key)) return NULL;
  else {
    Node *n = luaH_mainposition(t, key);
    do {  /* check whether `key' is somewhere in the chain */
      if (luaO_rawequalObj(gkey(n), key)) return gkey(n);  /* that's it */
      else n = n->next;
    } while (n);
    return NULL;;
  }
...
*/
const TObject *luaH_getkeynum (Table *t, int key) {
...
*/
const TObject *luaH_getkeystr (Table *t, TString *key) {
...
const TObject *luaH_getkeywstr (Table *t, TString *key) {
  Node *n = hashstr(t, key);
  do {  /* check whether `key' is somewhere in the chain */
    if (ttiswstring(gkey(n)) && tsvalue(gkey(n)) == key)
      return gkey(n);  /* that's it */
    else n = n->next;
  } while (n);
  return NULL;
...
const TObject *luaH_getkey (Table *t, const TObject *key) {
  switch (ttype(key)) {
    case LUA_TSTRING: return luaH_getkeystr(t, tsvalue(key));
    case LUA_TWSTRING: return luaH_getkeywstr(t, twsvalue(key));
    case LUA_TNUMBER: {
      int k;
      lua_number2int(k, (nvalue(key)));
    ...
    default: return luaH_getkeyany(t, key);
  }
}
...
const TObject *luaH_getwstr (Table *t, TString *key) {
  Node *n = hashstr(t, key);
  do {  /* check whether `key' is somewhere in the chain */
    if (ttiswstring(gkey(n)) && tsvalue(gkey(n)) == key)
      return gval(n);  /* that's it */
...
const TObject *luaH_get (Table *t, const TObject *key) {
  switch (ttype(key)) {
    ...
    case LUA_TWSTRING: return luaH_getwstr(t, twsvalue(key));
    case LUA_TNUMBER: {
      int k;
      lua_number2int(k, (nvalue(key)));
      if (cast(lua_Number, k) == nvalue(key))  /* is an integer index? */
...
TObject *luaH_setnum (lua_State *L, Table *t, int key) {
  ...
  if (p != &luaO_nilobject)
    ...
    TObject k;
    setnvalue2n(&k, cast(lua_Number, key));
    return newkey(L, t, &k);
  }
}
