/*
** $Id: lvm.c,v 1.1 2006/03/17 08:47:36 chaojin Exp $
** Lua virtual machine
...
static const TObject *luaV_getnotable (lua_State *L, const TObject *t,
  ...
  if (ttisnil(tm))
  {
	sethvalue(&foundObj, luaT_getmetatable(L, t));
	tm = &foundObj;
  ...
  if (useType == LUA_TSTRING)
  {
    ...
    do {
      ...
      if (!towstring(L, top-2) || !towstring(L, top-1)) {
        ...
        char *buffer;
        int i;
        while (n < total && towstring(L, top-n-1)) {  /* collect total length */
          tl += tsvalue(top-n-1)->tsv.len;
          n++;
		}
        if (tl > MAX_SIZET) luaG_runerror(L, "string size overflow");
        buffer = (char*)luaZ_openspace(L, &G(L)->buff, tl * 2);
        tl = 0;
        for (i=n; i>0; i--) {  /* concat all strings */
          size_t l = tsvalue(top-i)->tsv.len;
        ...
        setwsvalue(top-n, luaS_newlwstr(L, (const lua_WChar*)buffer, tl));
	  }
      total -= n-1;  /* got `n' strings to create 1 new */
      last -= n-1;
	} while (total > 1);  /* repeat until only 1 result left */
  ...
  for (;;) {
    ...
    switch (GET_OPCODE(i)) {
      ...
      case OP_CLOSURE: {
        ...
        ncl->l.p = p;
#if LUA_REFCOUNT        
		__AddRefDirect(ncl->l.p);
#endif LUA_REFCOUNT		
        for (j=0; j<nup; j++, pc++) {
          if (GET_OPCODE(*pc) == OP_GETUPVAL) {
            ncl->l.upvals[j] = cl->upvals[GETARG_B(*pc)];
			__AddRefDirect(ncl->l.upvals[j]);
		  }
          else {
