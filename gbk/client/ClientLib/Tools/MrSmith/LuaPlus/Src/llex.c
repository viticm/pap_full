...
/*
** $Id: llex.c,v 1.1 2006/03/17 08:47:35 chaojin Exp $
** Lexical Analyzer
...
#ifdef _WIN32_WCE
#define lex_isalpha(c)			( ('a' <= (c) && (c) <= 'z') || ('A' <= (c) && (c) <= 'Z') )
#define lex_isdigit(c)			( '0' <= (c) && (c) <= '9' )
#define lex_isalnum(c)			( lex_isalpha(c) || lex_isdigit(c) )
#else !_WIN32_WCE
...
#define wsave(LS, c, l) \
	(*(lua_WChar*)(luaZ_buffer((LS)->buff) + l) = cast(lua_WChar, c)), l += 2
#define wsave_and_next(LS, l)  (wsave(LS, LS->current, l), next(LS))
