/*
** $Id: liolib.c,v 1.1 2006/03/17 08:47:36 chaojin Exp $
...
static int uread_number (lua_State *L, FILE *f) {
  lua_Number d;
  if (fwscanf(f, LUA_NUMBER_WSCAN, &d) == 1) {
    lua_pushnumber(L, d);
    return 1;
  }
  else return 0;  /* read fails */
...
static int uread_line (lua_State *L, FILE *f) {
  luaL_Buffer b;
  luaL_wbuffinit(L, &b);
  for (;;) {
    size_t l;
    lua_WChar *p = (lua_WChar*)luaL_prepbuffer(&b);
    if (fgetws(p, LUAL_BUFFERSIZE / 2, f) == NULL) {  /* eof? */
      luaL_pushresult(&b);  /* close buffer */
      return (lua_strlen(L, -1) > 0);  /* check whether read something */
    }
    l = lua_WChar_len(p);
    if (p[l-1] != '\n')
      ...
      luaL_addwsize(&b, l - 1);  /* do not include `eol' */
      luaL_pushresult(&b);  /* close buffer */
      return 1;  /* read at least an `eol' */
    }
  }
...
static int uread_chars (lua_State *L, FILE *f, size_t n) {
  size_t rlen;  /* how much to read */
  size_t nr;  /* number of chars actually read */
  luaL_Buffer b;
  luaL_wbuffinit(L, &b);
  rlen = LUAL_BUFFERSIZE / 2;  /* try to read that much each time */
  do {
    char *p = luaL_prepbuffer(&b);
    if (rlen > n) rlen = n;  /* cannot read more than asked */
    nr = fread(p, sizeof(lua_WChar), rlen, f);
    luaL_addwsize(&b, nr);
    n -= nr;  /* still have to read `n' chars */
  } while (n > 0 && nr == rlen);  /* until end of count or eof */
  luaL_pushresult(&b);  /* close buffer */
...
static int g_write (lua_State *L, FILE *f, int arg) {
  ...
  for (; nargs--; arg++) {
    ...
    else if (lua_type(L, arg) == LUA_TWSTRING) {
      size_t l;
      const lua_WChar *s = luaL_checklwstring(L, arg, &l);
      status = status && (fwrite(s, sizeof(lua_WChar), l, f) == l);
    }
  }
