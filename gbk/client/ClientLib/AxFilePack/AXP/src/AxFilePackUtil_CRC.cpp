...
#define CRC_MAX_BUFFER_SIZE		(4096)
...
inline void _calcCrc32(const BYTE byte, unsigned int &dwCrc32)
{
...
bool crcMemory32(const void* pMemoryBuf, unsigned int dwBufSize, unsigned int& dwCrc32)
{
	...
	try
	{
		...
		for(dwLoop = 0; dwLoop < dwBufSize; dwLoop++)
			_calcCrc32(((BYTE*)pMemoryBuf)[dwLoop], dwCrc32);
	}
	catch(...)
	{
...
bool crcFile32(const char* szFilename, unsigned int &dwCrc32, bool bIgnoreTail)
{
	assert(szFilename);
	...
	try
	{
		...
		else
		{
			...
			BYTE buffer[CRC_MAX_BUFFER_SIZE];
			...
			while(bSuccess && dwBytesRead)
			{
				...
				if(bIgnoreTail && dwFileSize-dwBytesReadTotal <= 4)
				{
					dwBytesRead -= (dwBytesRead>4) ? (4-(dwFileSize-dwBytesReadTotal)) : dwBytesRead;
					bReachEnd = true;
				}

				for(dwLoop = 0; dwLoop < dwBytesRead; dwLoop++)
				{
					_calcCrc32(buffer[dwLoop], dwCrc32);
				}
