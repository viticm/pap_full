	...
	case DB_LOAD:
		{
			...
			INT iFriendSize = 0;
			INT iBlackSize  = 0;
			INT iGroupFriendSize[RELATION_GROUP_NUMBER] = {0};
			INT  FType;
				...
				else
				{
					...
					if(iFriendSize<RELATION_BLACKNAME_OFFSET)
					{
						...
						pCharFullData->m_Relation.m_aRelation[iFriendSize].m_Type			
							= mInterface->GetInt(DB_ReFlag,ErrorCode);
						BYTE nGroup = mInterface->GetBYTE(DB_GroupID,ErrorCode);
						pCharFullData->m_Relation.m_aRelation[iFriendSize].m_Group		
							= nGroup;

						if(nGroup>=RELATION_GROUP_F1 && nGroup<=RELATION_GROUP_F4)
							iGroupFriendSize[nGroup]++;
			...
			do 
			{
				...
				for(INT i =0;i<1;i++)
				{
					...
					// 重新计算各种关系的人数[7/24/2007]
					pCharFullData->m_Relation.m_aRelationCount[RELATION_GROUP_FRIEND_ALL]	= iFriendSize;
					pCharFullData->m_Relation.m_aRelationCount[RELATION_GROUP_F1]			= iGroupFriendSize[RELATION_GROUP_F1];
					pCharFullData->m_Relation.m_aRelationCount[RELATION_GROUP_F2]			= iGroupFriendSize[RELATION_GROUP_F2];
					pCharFullData->m_Relation.m_aRelationCount[RELATION_GROUP_F3]			= iGroupFriendSize[RELATION_GROUP_F3];
					pCharFullData->m_Relation.m_aRelationCount[RELATION_GROUP_F4]			= iGroupFriendSize[RELATION_GROUP_F4];
					pCharFullData->m_Relation.m_aRelationCount[RELATION_GROUP_BLACK]		= iBlackSize;
