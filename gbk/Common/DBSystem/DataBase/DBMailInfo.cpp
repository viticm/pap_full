...
BOOL DBMailInfo::Save(VOID* pSource)
{
		...
		for(INT i = 0 ;i<SmuCount;i++)
	{
			...
			// 含有非法存盘字符的邮件不存盘[3/12/2007]
			if(!StrSafeCheck(SourName,MAX_CHARACTER_NAME))
				continue;

			if(!StrSafeCheck(DestName,MAX_CHARACTER_NAME))
				continue;

			if(!StrSafeCheck(Context,MAX_MAIL_CONTEX))
				continue;

			//继续存储
			mInterface->Clear();
			CheckDBConnect("DBMailInfo");

			CHAR	MailInfo[200];
			memset(MailInfo,0,200);
