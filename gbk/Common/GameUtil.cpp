...
VOID	sm_lock(CHAR& flag,CHAR type)
{
	...
	if(flag==SM_FREE)
	{
		...
		if(flag!=type)
		{
			MySleep(1);
#ifdef __LINUX__
			printf("lock fail %s,%s,%s",__FILE__,__LINE__,__PRETTY_FUNCTION__);
#endif
			goto bgsm_loop;
		}
		return;
...
//#ifdef __LINUX__
//
//// while(!atomic_dec_and_test(reinterpret_cast<atomic_t*>(&flag))) {
////		atomic_inc(reinterpret_cast<atomic_t*>(&flag));
////		MySleep(0);
////		printf("lock fails.....\r\n");
//	}
//
//#else
//	while(InterlockedCompareExchange(reinterpret_cast<LPLONG>(&flag), type, SM_FREE)!=SM_FREE){
//		printf("lock fails.....\r\n");
//		MySleep(0); 
//	}
//#endif
...
//	//atomic_inc(reinterpret_cast<atomic_t*>(&flag));
//#else
//	while(InterlockedCompareExchange(reinterpret_cast<LPLONG>(&flag), SM_FREE, type)!=type)
//	{
//		printf("unlock fails.....\r\n");
//		MySleep(0);
//	}
...
VOID  DumpStack(const CHAR* type)
{
	...
	void *	DumpArray[256];
	int	Size =	backtrace(DumpArray,256);
	...
	if(symbols)
	{
		...
		if(Size>0)
		{
			...
			sprintf(threadinfo,"Dump: threadid=%d, Time: %d-%d-%d %d:%d:%d\r\n",
...
*/
UINT	g_CurWorld=0 ;
UINT	FullDataCrc(FULLUSERDATA* pData, BOOL bLog)
{
	Assert(pData);

	//g_pLog->SaveLog("./Log/Crc32.log",">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>GUID(%X)",pData->m_Human.m_GUID);
	//if(bLog)	printf("GUID=%X(%d)\n", pData->m_Human.m_GUID,pData->m_Human.m_GUID);
	UINT TotalCrc = 0;
	////m_Human
	//if(bLog)
	//{
	//	printf("start of m_Human-------------------------------------------\n");
	//}
	//TotalCrc += pData->m_Human.GetCrc(bLog);
	//if(bLog)
	//{
	//	printf("curcrc=%u\n",TotalCrc);
	//	printf("start of m_Bank--------------------------------------------\n");
	//}
	////m_Bank
	//TotalCrc += pData->m_Bank.GetCrc(bLog);
	//if(bLog)	
	//{
	//	printf("curcrc=%u\n",TotalCrc);
	//	printf("start of m_Bag---------------------------------------------\n");
	//}
	////m_Bag
	//TotalCrc += pData->m_Bag.GetCrc(bLog);
	//if(bLog)	
	//{
	//	printf("curcrc=%u\n",TotalCrc);
	//	printf("start of m_Equip-------------------------------------------\n");
	//}
	////m_Equip
	//TotalCrc += pData->m_Equip.GetCrc(bLog);
	//if(bLog)	
	//{
	//	printf("curcrc=%u\n",TotalCrc);
	//	//printf("start of m_Ability-----------------------------------------\n");
	//	printf("start of m_PetList-----------------------------------------\n");
	//}
	//m_Ability
	//TotalCrc += pData->m_Ability.GetCrc(bLog);
	//if(bLog)	
	//{
	//	printf("curcrc=%u\n",TotalCrc);
	//	printf("start of m_Skill-------------------------------------------\n");
	//}
	//m_Skill
	//TotalCrc += pData->m_Skill.GetCrc(bLog);
	//if(bLog)
	//{
	//	printf("curcrc=%u\n",TotalCrc);
	//	printf("start of m_XinFa-------------------------------------------\n");
	//}
	//m_XinFa
	//TotalCrc += pData->m_XinFa.GetCrc(bLog);
	//if(bLog)
	//{
	//	printf("curcrc=%u\n",TotalCrc);
	//	printf("start of m_Mission-----------------------------------------\n");
	//}
	//m_Mission
	//TotalCrc += pData->m_Mission.GetCrc(bLog);
	//if(bLog)	
	//{
	//	printf("curcrc=%u\n",TotalCrc);
	//	printf("start of m_Setting-----------------------------------------\n");
	//}
	//m_Setting
	//TotalCrc += pData->m_Setting.GetCrc(bLog);
	//if(bLog)
	//{
	//	printf("curcrc=%u\n",TotalCrc);
	//	printf("start of m_PetList-----------------------------------------\n");
	//}
	//m_PetList
	//TotalCrc += pData->m_PetList.GetCrc(bLog);
	//if(bLog)
	//{
	//	printf("curcrc=%u\n",TotalCrc);
	//	printf("start of m_Relation----------------------------------------\n");
	//}
	//m_Relation
	//TotalCrc += pData->m_Relation.GetCrc(bLog);
	//if(bLog)
	//{
	//	printf("curcrc=%u\n",TotalCrc);
	//	printf("start of m_PrivateInfo-------------------------------------\n");
	//}
	//m_PrivateInfo
	//TotalCrc += pData->m_PrivateInfo.GetCrc(bLog);
	//if(bLog)
	//{
	//	printf("curcrc=%u\n",TotalCrc);
	//}
	//g_pLog->SaveLog("./Log/Crc32.log",">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	TotalCrc += g_CurWorld;
	if(bLog)
	{
		printf("curcrc=%u\n",TotalCrc);
		printf("start of g_CurWorld----------------------------------------\n");
	}
...
{
	//0~9
	const CHAR	numBegin	= 0x30;
	const CHAR	numEnd		= 0x39;
	//A~Z
	const CHAR	ascBegin1	= 0x41;
	const CHAR	ascEnd1		= 0x5A;
	//a~z
	const CHAR	ascBegin2	= 0x61;
	const CHAR	ascEnd2		= 0x7A;
	//_
	const CHAR	ascUnder	= 0x5F;
	//@
	const CHAR	ascEmail	= 0x40;

#define		IsNum(x)	((numBegin <= (x) && numEnd >= (x))?TRUE:FALSE)
#define		IsAsc(x)	(((ascBegin1 <= (x) && ascEnd1 >= (x)) || (ascBegin2 <= (x) && ascEnd2 >= (x)))?TRUE:FALSE)
#define		IsUnder(x)	((ascUnder == (x))?TRUE:FALSE)
#define		IsEmail(x)	((ascEmail == (x))?TRUE:FALSE)

#define		MinNameLength		1
#define		MaxNameLength		16
#define		MaxTotalNameLength	50

	switch(nLanguage)
	{
	case 1://cn
		{
			//if(FALSE == StrSafeCheck(pIn,InLength))	return FALSE;
			//首字符必须是字母或数字
			//if(FALSE == IsNum(pIn[0]) && FALSE == IsAsc(pIn[0]))	return FALSE;
			//其他字符必须是字母、数字和下划线
			UINT nNameLength	= 0;
			for(UINT i = 0; i < InLength; ++i)
			{
				if(0 == pIn[i] || IsEmail(pIn[i]))	break;
				//if(FALSE == IsNum(pIn[i]) && FALSE == IsAsc(pIn[i]) && FALSE == IsUnder(pIn[i]))	return FALSE;
				nNameLength++;
			}
			//用户名5-16位字符
			if(MinNameLength > nNameLength || MaxNameLength < nNameLength)	return FALSE;
		}
		break;
	case 0://en
	case 2://tw
	...
	default:
		{
			//总长度不大于50
			if(MinNameLength > strlen(pIn) || MaxTotalNameLength < strlen(pIn))	return FALSE;
		}
		break;
	}
...
//由于Linux下unixODBC-2.2.11里SQLAllocHandle并发调用时
//会产生SIGABRT，故添加此变量控制并发
//0代表没有线程在使用，非0代表有线程在使用
INT					g_DBConnectThread=0;