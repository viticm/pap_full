...
namespace Packets
{
	...
	public:
		...
		GUID_t				GetGUID(VOID)	const	{return m_nGuid;}
		VOID			SetGUID(GUID_t	guid){ m_nGuid	=	guid; SetUpdateAttrib(DETAIL_ATTRIB_GUID,TRUE); }
        ...
        //  [7/23/2010 陈军龙]
		INT				GetMountIndex(VOID)	const	{return m_nMountIndex;}
		VOID			SetMountIndex(INT	nIndex)		{m_nMountIndex = nIndex; SetUpdateAttrib(DETAIL_ATTRIB_MOUNTINDEX,TRUE);}
	...
	private:
		...
		INT				m_nVigor_ReSpeed;	//活力恢复速度
		//INT				m_nAtt_Physics;		//物理攻击力

		INT				m_nAtt_Near;        //近程攻击
		INT             m_nAtt_Far;         //远程攻击

		INT				m_nAtt_MagicNear;	//近程内功攻击
		INT				m_nAtt_MagicFar;	//近程外功攻击
