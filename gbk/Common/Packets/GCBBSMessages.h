...
namespace Packets
{
	...
	public:
		...
		VOID					SetTitleLength(BYTE nTitleLength) 
		{
			if (nTitleLength>=MAX_BBS_MESSAGE_LENGTH)
			{
				nTitleLength = MAX_BBS_MESSAGE_LENGTH-1;
			}
			m_nTitleLength = nTitleLength;
		};

		CHAR*					GetTitle(VOID) {return m_szTitle;}
		VOID					SetTitle(CHAR* pszTitle)
		{
			if (m_nTitleLength>=MAX_BBS_MESSAGE_LENGTH)
			{
				m_nTitleLength = MAX_BBS_MESSAGE_LENGTH-1;
			}
			memcpy(m_szTitle, pszTitle, m_nTitleLength);
			m_szTitle[m_nTitleLength] = 0;
		}
		...
		VOID					SetMessageData(_MESSAGE_T* pSour)
		{
			...
			for(UINT i = 0; i<m_MessageNum; i++)
			{
				if(m_MessageData[i].bHasReply)
				{
					if (m_MessageData[i].nMsgLength>=MAX_BBS_MESSAGE_LENGTH)
					{
						m_MessageData[i].nMsgLength = MAX_BBS_MESSAGE_LENGTH-1;
					}
					if (m_MessageData[i].nReplyMsgLength>=MAX_BBS_MESSAGE_LENGTH)
					{
						m_MessageData[i].nReplyMsgLength = MAX_BBS_MESSAGE_LENGTH-1;
					}
					m_MessageData[i].szMessage[m_MessageData[i].nMsgLength] = 0;
					m_MessageData[i].szReplyMessage[m_MessageData[i].nReplyMsgLength] = 0;
				}
				else
				{
					if (m_MessageData[i].nMsgLength>=MAX_BBS_MESSAGE_LENGTH)
