...
namespace Packets
{
	...
	public:
		...
		VOID					SetLength(BYTE	nLength)
		{
			if (nLength>=MAX_BBS_MESSAGE_LENGTH)
			{
				nLength = MAX_BBS_MESSAGE_LENGTH - 1;
			}
			m_nLength = nLength;
		}
		...
		{
			if (m_nLength>=MAX_BBS_MESSAGE_LENGTH)
			{
				m_nLength = MAX_BBS_MESSAGE_LENGTH - 1;
			}
			memcpy(m_MessageData, pSour, m_nLength);
			m_MessageData[m_nLength] = 0;
		}
