...
BOOL CLAskCreateChar::Read( SocketInputStream& iStream ) 
{
	__ENTER_FUNCTION
		...
		iStream.Read((CHAR*)&m_Profession,sizeof(BYTE));
		iStream.Read((CHAR*)&m_Country,sizeof(BYTE));
...
BOOL CLAskCreateChar::Write( SocketOutputStream& oStream )const
{
	__ENTER_FUNCTION
		...
		oStream.Write((CHAR*)&m_Profession,sizeof(BYTE));
		oStream.Write((CHAR*)&m_Country,sizeof(BYTE));
