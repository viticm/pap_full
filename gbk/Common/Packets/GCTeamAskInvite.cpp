...
__ENTER_FUNCTION
	...
	for( INT i=0; i<m_uMemberSize; ++i )
	{
		...
		if ( m_InviterInfo[i].m_DetailFlag>0 )
		{
			...
			iStream.Read( (CHAR*)&m_InviterInfo[i].m_AssihandID, sizeof(UINT) );
			iStream.Read( (CHAR*)&m_InviterInfo[i].m_CapID, sizeof(UINT) );
			iStream.Read( (CHAR*)&m_InviterInfo[i].m_ScapularID, sizeof(UINT) );
...
__ENTER_FUNCTION
	...
	for( INT i=0; i<m_uMemberSize; ++i )
	{
		...
		if ( m_InviterInfo[i].m_DetailFlag>0 )
		{
			...
			oStream.Write( (CHAR*)&m_InviterInfo[i].m_AssihandID, sizeof(UINT) );
			oStream.Write( (CHAR*)&m_InviterInfo[i].m_CapID, sizeof(UINT) );
			oStream.Write( (CHAR*)&m_InviterInfo[i].m_ScapularID, sizeof(UINT) );
