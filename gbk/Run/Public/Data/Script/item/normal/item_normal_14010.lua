--脚本号
x414010_g_scriptId = 414010 --临时写这个,真正用的时候一定要改.
x414010_g_keyID = 12100010 --临时写这个,真正用的时候一定要改.
x414010_g_keyname = "钥匙（10级）" --临时写这个,真正用的时候一定要改.
...
function x414010_OnDefaultEvent( sceneId, selfId, bagIndex )
	...
	if HaveKey > 0 then
		...
		BeginEvent(sceneId)
			strText = "#Y你使用一把"..x414010_g_keyname.."，开启了一个宝箱"
			AddText(sceneId,strText);
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	else
		BeginEvent(sceneId)
			strText = "#Y开启宝箱，需要"..x414010_g_keyname.."一把"
			AddText(sceneId,strText);
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	end
