--脚本号
x414001_g_scriptId = 414001 --临时写这个,真正用的时候一定要改.
x414001_g_keyID = 12100001 --临时写这个,真正用的时候一定要改.
x414001_g_keyname = "钥匙（1级）" --临时写这个,真正用的时候一定要改.
...
function x414001_OnDefaultEvent( sceneId, selfId, bagIndex )
	...
	if HaveKey > 0 then
		...
		BeginEvent(sceneId)
			strText = "#Y你使用一把"..x414001_g_keyname.."，开启了一个宝箱"
			AddText(sceneId,strText);
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	else
		BeginEvent(sceneId)
			strText = "#Y开启宝箱，需要"..x414001_g_keyname.."一把"
			AddText(sceneId,strText);
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	end
