--脚本号
x414007_g_scriptId = 414007 --临时写这个,真正用的时候一定要改.
x414007_g_keyID = 12100007 --临时写这个,真正用的时候一定要改.
x414007_g_keyname = "钥匙（7级）" --临时写这个,真正用的时候一定要改.
...
function x414007_OnDefaultEvent( sceneId, selfId, bagIndex )
	...
	if HaveKey > 0 then
		...
		BeginEvent(sceneId)
			strText = "#Y你使用一把"..x414007_g_keyname.."，开启了一个宝箱"
			AddText(sceneId,strText);
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	else
		BeginEvent(sceneId)
			strText = "#Y开启宝箱，需要"..x414007_g_keyname.."一把"
			AddText(sceneId,strText);
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	end
