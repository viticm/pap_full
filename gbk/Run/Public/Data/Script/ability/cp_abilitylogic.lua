...
-------------------------------------------------------------------------
-- 合成结束时的成功率计算
function CheckForResult(sceneId, selfId, AbilityID, RecipeLevel)
	-- 辅助生活技能级别
...
---------------------------------------------------------------------
-- 采集结束时的熟练度增长
function x701601_GatherGainExperience(sceneId, selfId, AbilityID, RecipeLevel)
	-- 生活技能级别
	...
	if AbilityLevel < 1 or AbilityLevel > 10 then
		return
	end
	
	addExp = GetAbilityExpPlus(sceneId, selfId, 0);
	SetAbilityExp(sceneId, selfId, AbilityID, addExp)
	return
end

-- 配方合成结束时的熟练度增长
function x701601_CompoundGainExperience(sceneId, selfId, AbilityID, RecipeLevel)
	-- 生活技能级别
	local AbilityLevel = QueryHumanAbilityLevel(sceneId, selfId, AbilityID)
	local MinLevelDisparity = 0
	local MaxLevelDisparity = 1
...
--宝石摘除接口
--返回值 0:成功 1:无此装备 2:宝石位置错误 3:装备上没有宝石 4:装备非法 8:需要宝石摘除符
function	x701601_ReomveProc(sceneId,selfId, EquipIndex, GemIndex, MatIndex)
	...
	if gem_index ~= 30900012 then --宝石摘除符
		return 8
	end
