...
x700102_g_CopySceneName				= "浮屠塔二层"
x700102_g_Name								= ""
x700102_g_CopySceneType				= FUBEN_FUTUTA
x700102_g_LimitMembers				= 1			--可以进副本的最小队伍人数
x700102_g_TickTime						= 5				--回调脚本的时钟时间（单位：秒/次）
x700102_g_LimitTotalHoldTime	= 360	--副本可以存活的时间（单位：次数）,如果此时间到了，则任务将会失败
x700102_g_CloseTick						= 2				--副本关闭前倒计时（单位：次数）
x700102_g_NoUserTime					= 10			--副本中没有人后可以继续保存的时间（单位：秒）
x700102_g_DeadTrans						= 0				--死亡转移模式，0：死亡后还可以继续在副本，1：死亡后被强制移出副本
x700102_g_Fuben_X							= 63				--进入副本的位置X
x700102_g_Fuben_Z							= 98				--进入副本的位置Z
x700102_g_Back_X							= 433				--源场景位置X
x700102_g_Back_Z							= 422					--源场景位置Z

x700102_g_MissionName				= "浮屠塔二层"
x700102_g_MissionInfo				= "杀死全部怪物！"
x700102_g_MissionTarget			= "杀死全部怪物！"
x700102_g_MissionBonusInfo	= "获得%d的经验奖励。"
...
x700102_g_MissionComplete		= "谢谢啊，俺们终于敢出门了"
x700102_g_MissionFuBenOpen	= "传入副本。"

x700102_g_KillMonsterMessage		= "你已经杀死了%d/%d只怪！"
x700102_g_CompleteOutMessage		= "任务完成，将在%d秒后传送到下一层！"
...
x700102_g_ErrorMessage_Round		= format("你今天已经做过%d次了！", x700102_g_RoundUntil)

x700102_g_ErrorMessage_OtherLevel		= "%s的等级不足%d！"
x700102_g_ErrorMessage_OtherAround	= "%s不在你身边！"
x700102_g_ErrorMessage_OtherRound		= "%s今天已经做过%d次了！"
...
x700102_g_TimeOutFailedMessage	= "任务失败，超时！"
x700102_g_CompleteFailedNPCMessage	= "你们竟然失败了！"
x700102_g_TipForEnter						= "进入浮屠塔！"
x700102_g_SceneMapNavPath				= "fuben_fututaerceng_a/fuben_fututaerceng_a.nav"

x700102_g_BossEmergeMessage			= "BOSS出现！"
...
function x700102_MakeCopyScene(sceneId, selfId, memberNum, backsceneId)
	...
	for i, item in x700102_g_NeedKillMonster do
		if item.hash == 1 then
			ntotalmonster = ntotalmonster + 1;
		end
	end
	LuaFnSetCopySceneData_Param(sceneId, x700102_CSP_NTOTALMONSTER, ntotalmonster) ;--全部怪数量

	if backsceneId == nil then
		backsceneId = sceneId;
	end
	LuaFnSetCopySceneData_Param(sceneId, x700102_CSP_FROMSCENEID, backsceneId);--设置副本入口场景号
...
function x700102_OnKillObject(sceneId, selfId, objdataId ,objId)
	...
	for i = 0, humancount - 1 do
		...
		misIndex = GetMissionIndexByID(sceneId, humanId, x700102_g_MissionId); --取得任务数据索引值
		local killedcount = GetMissionParam(sceneId, humanId, misIndex, x700102_MP_KILLMONSTER) + 1; --取得已经杀了的怪物数
		SetMissionByIndex(sceneId, humanId, misIndex, x700102_MP_KILLMONSTER, killedcount); --设置任务数据
	end
end
