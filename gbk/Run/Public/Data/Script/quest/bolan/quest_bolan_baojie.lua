...
x226006_g_MissionName="报捷"
x226006_g_MissionInfo="     #G贝达尔#W应该还不知道亨利被杀的消息，要尽快让他知道这个消息，这对他歼灭波兰的步兵会有很大帮助。帮我把这个捷报带给他吧。"  --任务描述
x226006_g_MissionTarget="    告诉#G贝达尔#W亨利被杀的消息。"		
x226006_g_MissionComplete="    没有这个消息，我一样会歼灭波兰的步兵，不过这依旧是一个好消息。"					--完成任务npc说话的话
--x226006_g_ContinueInfo="    不管付出多大代价，也要把那件东西给拿回来。"
--任务奖励
...
function x226006_OnDefaultEvent(sceneId, selfId, targetId)	--点击该任务后执行此脚本
...
function x226006_OnAccept(sceneId, selfId)
	...
	AddText(sceneId,"接受任务："..x226006_g_MissionName) 
	EndEvent()
...
function x226006_OnAbandon(sceneId, selfId)
	...
	for i, item in pairs(x226006_g_DemandItem) do
...
function x226006_OnSubmit(sceneId, selfId, targetId, selectRadioId)
	if DelMission(sceneId, selfId, x226006_g_MissionId) > 0 then
		...
		for i, item in pairs(x226006_g_RadioItemBonus) do
