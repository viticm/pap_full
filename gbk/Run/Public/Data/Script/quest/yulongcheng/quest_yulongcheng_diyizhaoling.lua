...
x208000_g_MissionName="第一诏令"
x208000_g_MissionInfo="    邪恶的魔鬼进入了魔诃魔的躯体，他被贪婪和自大给占据了，他用那沾满鲜血的匕首在红沙漠里犯下了滔天大罪，他们抢走了我们的珍宝和马匹，杀死了我们450个同胞，他们藐视我长生天的勇士，即使连他们的真主也不会宽恕他们。花剌子模的人，一定要为这样邪恶的行径付出代价。\n \n    我--成吉思汗，向所有的勇士们发出第一诏令，不管是哪一国的人，不管你从哪里来，只要你能带着你的正义，让那些花剌子模人的血洒在大地上，你将会得到我最丰厚的奖赏。"  --任务描述
x208000_g_MissionTarget="    杀死20名#R花剌子模武士#W，回来跟#G成吉思汗#W领赏。"		                                                                                               
x208000_g_MissionComplete="    对，一定要用他们的血来祭奠那些死去的人！"					--完成任务npc说话的话
x208000_g_ContinueInfo="    请不要怜悯那些花剌子模人，只有杀了他们才能平息我们仇恨的怒火。"
--任务奖励
...
function x208000_OnDefaultEvent(sceneId, selfId, targetId)	--点击该任务后执行此脚本
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x208000_g_MissionId)
		if x208000_CheckSubmit(sceneId, selfId, targetId) <= 0 then
			...
			AddText(sceneId,format("\n    杀死花剌子模武士   %d/%d", GetMissionParam(sceneId,selfId,misIndex,0),x208000_g_DemandKill[1].num ))
			EndEvent()
...
function x208000_OnAccept(sceneId, selfId)
		...
		AddText(sceneId,"接受任务："..x208000_g_MissionName) 
		EndEvent()
...
function x208000_OnAbandon(sceneId, selfId)
	...
	for i, item in x208000_g_DemandItem do
...
function x208000_OnSubmit(sceneId, selfId, targetId, selectRadioId)
	...
	if DelMission(sceneId, selfId, x208000_g_MissionId) > 0 then
		...
		for i, item in x208000_g_RadioItemBonus do
	 ...
	 num = GetMissionParam(sceneId,selfId,misIndex,0) 
	 	 if objdataId == x208000_g_DemandKill[1].id then 
       		    if num < x208000_g_DemandKill[1].num then
       		         ...
       		         BeginEvent(sceneId)
			 AddText(sceneId,format("杀死花剌子模武士   %d/%d", GetMissionParam(sceneId,selfId,misIndex,0),x208000_g_DemandKill[1].num )) 
			 EndEvent()
			 DispatchMissionTips(sceneId, selfId)
		    end
       		 end
