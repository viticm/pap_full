...
x208010_g_MissionName="山谷屠杀"
x208010_g_MissionInfo="     依照父汗的三路进攻方案，我的第二路兵团攻下了讹答剌城，从侧面直取玉龙城，把玉龙城的精兵牢牢的困在了东北边的山谷里。\n \n    山谷的地势对守方有利，我还需要点时间才能把他们给歼灭。"  --任务描述
x208010_g_MissionInfo2="，我经常听到士兵们提起你，请你加入我们的大军，一起战斗吧。"
x208010_g_MissionTarget="    杀死20名#R玉龙城精兵#W，回来向#G窝阔台#W复命。"		                                                                                               
x208010_g_MissionComplete="    这些精兵真不好对付，幸亏有你的加入，等战事结束后，我要设全羊宴感谢你。"					--完成任务npc说话的话
x208010_g_ContinueInfo="    请不要怜悯那些花剌子模人，只有杀了他们才能平息我们仇恨的怒火。"
--任务奖励
...
function x208010_OnDefaultEvent(sceneId, selfId, targetId)	--点击该任务后执行此脚本
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x208010_g_MissionId)
		if x208010_CheckSubmit(sceneId, selfId, targetId) <= 0 then
			...
			AddText(sceneId,format("\n    杀死玉龙城精兵   %d/%d", GetMissionParam(sceneId,selfId,misIndex,0),x208010_g_DemandKill[1].num ))
			EndEvent()
...
function x208010_OnAccept(sceneId, selfId)
		...
		AddText(sceneId,"接受任务："..x208010_g_MissionName) 
		EndEvent()
...
function x208010_OnAbandon(sceneId, selfId)
	...
	for i, item in x208010_g_DemandItem do
...
function x208010_OnSubmit(sceneId, selfId, targetId, selectRadioId)
	...
	if DelMission(sceneId, selfId, x208010_g_MissionId) > 0 then
		...
		for i, item in x208010_g_RadioItemBonus do
	 ...
	 num = GetMissionParam(sceneId,selfId,misIndex,0) 
	 	 if objdataId == x208010_g_DemandKill[1].id then 
       		    if num < x208010_g_DemandKill[1].num then
       		         ...
       		         BeginEvent(sceneId)
			 AddText(sceneId,format("杀死玉龙城精兵   %d/%d", GetMissionParam(sceneId,selfId,misIndex,0),x208010_g_DemandKill[1].num )) 
			 EndEvent()
			 DispatchMissionTips(sceneId, selfId)
		    end
       		 end
