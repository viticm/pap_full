...
--任务是否已经完成
x910002_g_IsMissionOkFail = 0		--变量的第0位

--任务需要杀死的怪
x910002_g_DemandKill ={{id=1002,num=20}}		--变量第1位

--以上是动态**************************************************************

--任务文本描述
x910002_g_MissionName="与父汗决裂"
x910002_g_MissionInfo="虽然王罕是我的父汗，但是任何阻挡我统一蒙古的石头，我都要捏碎它！"  --任务描述
x910002_g_MissionTarget="杀死20个克烈兵"		--任务目标
x910002_g_ContinueInfo="你已经杀了20个克烈兵吗？"		--未完成任务的npc对话
x910002_g_MissionComplete="为了蒙古统一，即使是面对父汗，我也决不能手软！"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x910002_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
	...
	elseif x910002_CheckAccept(sceneId,selfId) > 0 then
		...
		BeginEvent(sceneId)
			...
			for i, item in x910002_g_ItemBonus do
				AddItemBonus( sceneId, item.id, item.num )
			end
			for i, item in x910002_g_RadioItemBonus do
...
function x910002_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x910002_g_MissionId, x910002_g_ScriptId, 1, 0, 0 )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x910002_g_MissionId)			--得到任务的序列号
 ...
 if objdataId == x910002_g_DemandKill[1].id then
	  ...
	  if num < x910002_g_DemandKill[1].num then
	    ...
	    SetMissionByIndex(sceneId,selfId,misIndex,1,num+1)
	  	BeginEvent(sceneId)
	  	strText = format("已杀死克烈兵%d/20", GetMissionParam(sceneId,selfId,misIndex,1) )
	  	AddText(sceneId,strText);
	  	EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	  end
