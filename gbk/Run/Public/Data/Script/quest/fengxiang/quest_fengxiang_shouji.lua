...
--任务是否已经完成
x910011_g_IsMissionOkFail = 0		--变量的第0位

--任务需要收集的物品
x910011_g_DemandItem ={{id=40001001,num=5}}		--变量第1位

--以上是动态**************************************************************

--任务文本描述
x910011_g_MissionName="收集令牌"
x910011_g_MissionInfo="蒙古勇士牺牲的时候，令牌都被敌人搜去了。我需要你拿回这些令牌来，以慰勇士之灵！"  --任务描述
x910011_g_MissionTarget="收集5个蒙古军队令牌"		--任务目标
x910011_g_ContinueInfo="你有5个蒙古军队令牌吗？"		--未完成任务的npc对话
x910011_g_MissionComplete="这些牺牲的勇士，一定能得到长生天的眷顾的！"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x910011_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
	...
	if IsHaveMission(sceneId,selfId,x910011_g_MissionId) > 0 then
		...
		BeginEvent(sceneId)
			...
			for i, item in pairs(x910011_g_DemandItem) do
	...
	elseif x910011_CheckAccept(sceneId,selfId) > 0 then
		...
		BeginEvent(sceneId)
			...
			for i, item in pairs(x910011_g_ItemBonus) do
				AddItemBonus( sceneId, item.id, item.num )
			end
			for i, item in pairs(x910011_g_RadioItemBonus) do
...
function x910011_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x910011_g_MissionId, x910011_g_ScriptId, 1, 0, 0 )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x910011_g_MissionId)			--得到任务的序列号
