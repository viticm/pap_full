...
--任务是否已经完成
x970017_g_IsMissionOkFail = 0		--变量的第0位

x970017_g_DemandItem ={}

--任务名称
x970017_g_MissionName="搜侠录"
--任务描述(接任务时，和所有目标完成交任务时)支持多条语句
x970017_g_MissionInfo={"#{quest_souxialu}"} 

x970017_g_MissionTarget="与多隆继续谈话"	

--任务未完成时去交任务的提示文字 ，支持多条语句
x970017_g_UnDoneMissionInfo={"与我继续交谈吧"}

x970017_g_MissionComplete="明白了吧，多多利用吧，它会让你的旅程更加精彩。"
...
--任务描述(接任务时，和所有目标完成交任务时)支持多条语句
--x970017_g_UnDoneMissionInfo={"继续与我谈话吧"}  
--总的大体任务目标描述，一句话描述
x970017_g_MissionAim="与多隆继续谈话"		
--任务目标类型(可以多任务目标)


--DELIVERY						--送信                        --npc为要送信给的对象,item为是否有任务物品要给送信对象,该item由任务发布人直接给予玩家; info为任务目标显示
x970017_g_DELIVERY = {{npc = "多隆",item = 0,num = 0,comeitem = 0,comenum = 0,info="",dialog="",type="DELIVERY",order=0}}
...
--经验奖励
x970017_g_ExpBonustxt = "经验  1000+等级×等级"
x970017_g_exp = 1
...
function x970017_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
    ...
    if IsHaveMission(sceneId,selfId,x970017_g_MissionId) > 0 then
		if (GetName(sceneId,targetId)==x970017_g_NameEnd) then
			local m = 0
			for i, QuestInfo in x970017_g_Quest do
				if (QuestInfo.type == "DELIVERY") then
					if (getn(x970017_g_DELIVERY)==1) and (x970017_g_NameEnd == x970017_g_DELIVERY[1].npc) then	
						m = 2
					else
						m = 1
					end
		...
		elseif (x970017_g_DELIVERY ~= nil) then
			...
			if  n == 1 then
				for i, DeliveryInfo in x970017_g_DELIVERY do
					if (GetName(sceneId,targetId)==DeliveryInfo.npc)then
						...
						elseif m == 0 then
							BeginEvent(sceneId)
								AddText(sceneId,x970017_g_Name.."让你带给我的东西在哪呢？")
							EndEvent( )	
							DispatchEventList(sceneId,selfId,targetId)
						end
					end
...
function	x970017_ShowQuestInfo( sceneId, selfId, targetId ,Done)
	...
	AddText(sceneId,"#Y"..x970017_g_MissionName)
	if DoneEX==2 then
		for i, Info in x970017_g_ContinueInfo do
			AddText(sceneId,Info)
		end
	else
		for i, Info in x970017_g_MissionInfo do
			AddText(sceneId,Info)
		end
	end
	...
	if (GetName(sceneId,targetId)==x970017_g_Name) then
		if (GetLevel( sceneId, selfId ) >= x970017_g_MissionMinLevel ) and (GetLevel( sceneId, selfId ) <= x970017_g_MissionMaxLevel ) then
			if	x970017_g_MisIdPre == nil then
				...
				for i, questpre in x970017_g_MisIdPre do
...
function x970017_OnAccept(sceneId, selfId )		
	...
	for i, QuestInfo in x970017_g_Quest do
		if (QuestInfo.type == "DELIVERY") then
				...
				local m = 0
				local ret = 1
				for i, QuestLableInfo in x970017_g_Quest do
	...
	AddMission( sceneId,selfId, x970017_g_MissionId, x970017_g_ScriptId, DoKill, DoArea, DoItem )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x970017_g_MissionId)			--得到任务的序列号
	...
	BeginEvent(sceneId)
	  	strText = "你接受了任务  "..x970017_g_MissionName
	  	AddText(sceneId,strText);
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
end
...
function x970017_OnSubmit( sceneId, selfId, targetId,selectRadioId )
	...
	if (x970017_CheckSubmit( sceneId, selfId ) == 1) then
			...
			if ret > 0 then
				for i, QuestInfo in x970017_g_Quest do
					if QuestInfo.type ==  "DELIVERY" then
						if getn(x970017_g_DELIVERY) == 1 and QuestInfo == x970017_g_DELIVERY[1] then
				...
				BeginEvent(sceneId)
				  	strText = "你完成了任务  "..x970017_g_MissionName
				  	AddText(sceneId,strText);
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,selfId)
