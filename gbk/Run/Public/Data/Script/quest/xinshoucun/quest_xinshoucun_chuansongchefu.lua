...
--任务是否已经完成
x970021_g_IsMissionOkFail = 0		--变量的第0位

x970021_g_DemandItem ={}

--任务名称
x970021_g_MissionName="传送车夫"
--任务描述(接任务时，和所有目标完成交任务时)支持多条语句
x970021_g_MissionInfo={"到底发生什么事了？我现在也不知道更多了，你应该去大城市打听打听。不过我会一直支持你的。不要怕，你现在已经像一个大侠了。到村子南边，那里有个车夫常年往返村子到苏州两地，和他谈谈，他会带你去苏州的。"} 

x970021_g_MissionTarget="去驿站跟#aB{60,238,3,杨柳村,驿站}[车夫]#aE交谈，他会将你送到苏州城"	

--任务未完成时去交任务的提示文字 ，支持多条语句
x970021_g_UnDoneMissionInfo={"找驿站问问情况吧"}

x970021_g_MissionComplete="你来啦！我就知道你会来找我的.帮村子做了这么多，我都看到了。我可以送您去苏州见您要找的人。"
...
--任务描述(接任务时，和所有目标完成交任务时)支持多条语句
--x970021_g_UnDoneMissionInfo={"找多隆问问是什么情况"}  
--总的大体任务目标描述，一句话描述
x970021_g_MissionAim="与多隆谈话"		
--任务目标类型(可以多任务目标)


--DELIVERY						--送信                        --npc为要送信给的对象,item为是否有任务物品要给送信对象,该item由任务发布人直接给予玩家; info为任务目标显示
x970021_g_DELIVERY = {{npc = "驿站",item = 0,num = 0,comeitem = 0,comenum = 0,info="",dialog="",type="DELIVERY",order=0}}
...
--经验奖励
x970021_g_ExpBonustxt = "经验  1000+等级×等级"
x970021_g_exp = 1
...
function x970021_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
    ...
    if IsHaveMission(sceneId,selfId,x970021_g_MissionId) > 0 then
		if (GetName(sceneId,targetId)==x970021_g_NameEnd) then
			local m = 0
			for i, QuestInfo in pairs(x970021_g_Quest) do
				if (QuestInfo.type == "DELIVERY") then
					if (#(x970021_g_DELIVERY)==1) and (x970021_g_NameEnd == x970021_g_DELIVERY[1].npc) then	
						m = 2
					else
						m = 1
					end
		...
		elseif (x970021_g_DELIVERY ~= nil) then
			...
			if  n == 1 then
				for i, DeliveryInfo in pairs(x970021_g_DELIVERY) do
					if (GetName(sceneId,targetId)==DeliveryInfo.npc)then
						...
						elseif m == 0 then
							BeginEvent(sceneId)
								AddText(sceneId,x970021_g_Name.."让你带给我的东西在哪呢？")
							EndEvent( )	
							DispatchEventList(sceneId,selfId,targetId)
						end
					end
...
function	x970021_ShowQuestInfo( sceneId, selfId, targetId ,Done)
	...
	AddText(sceneId,"#Y"..x970021_g_MissionName)
	if DoneEX==2 then
		for i, Info in pairs(x970021_g_ContinueInfo) do
			AddText(sceneId,Info)
		end
	else
		for i, Info in pairs(x970021_g_MissionInfo) do
			AddText(sceneId,Info)
		end
	end
	...
	if (GetName(sceneId,targetId)==x970021_g_Name) then
		if (GetLevel( sceneId, selfId ) >= x970021_g_MissionMinLevel ) and (GetLevel( sceneId, selfId ) <= x970021_g_MissionMaxLevel ) then
			if	x970021_g_MisIdPre == nil then
				...
				for i, questpre in pairs(x970021_g_MisIdPre) do
...
function x970021_OnAccept(sceneId, selfId )		
	...
	for i, QuestInfo in pairs(x970021_g_Quest) do
		if (QuestInfo.type == "DELIVERY") then
				...
				local m = 0
				local ret = 1
				for i, QuestLableInfo in pairs(x970021_g_Quest) do
	...
	AddMission( sceneId,selfId, x970021_g_MissionId, x970021_g_ScriptId, DoKill, DoArea, DoItem )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x970021_g_MissionId)			--得到任务的序列号
	...
	BeginEvent(sceneId)
	  	strText = "你接受了任务  "..x970021_g_MissionName
	  	AddText(sceneId,strText);
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
end
...
function x970021_OnSubmit( sceneId, selfId, targetId,selectRadioId )
	...
	if (x970021_CheckSubmit( sceneId, selfId ) == 1) then
			...
			if ret > 0 then
				for i, QuestInfo in pairs(x970021_g_Quest) do
					if QuestInfo.type ==  "DELIVERY" then
						if #(x970021_g_DELIVERY) == 1 and QuestInfo == x970021_g_DELIVERY[1] then
				...
				BeginEvent(sceneId)
				  	strText = "你完成了任务  "..x970021_g_MissionName
				  	AddText(sceneId,strText);
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,selfId)
