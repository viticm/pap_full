...
--任务起始npc
x970007_g_Name	= "武器店掌柜"

--任务提交npc
x970007_g_NameEnd	= "药品店掌柜"
...
--任务是否已经完成
x970007_g_IsMissionOkFail = 0		--变量的第0位

x970007_g_DemandItem ={}

--任务名称
x970007_g_MissionName="礼物3— 救命之药"
--任务描述(接任务时，和所有目标完成交任务时)支持多条语句
x970007_g_MissionInfo={"#{quest_jiumingzhiyao}"} 

x970007_g_MissionTarget="去找#aB{98,171,3,杨柳村,药品店掌柜}[药店品掌柜]#aE领取救命的药"	

x970007_g_ContinueInfo={"你确定已经去过药品店了吗？快去吧，药品在这儿是很重要的东西。"}  

x970007_g_MissionComplete="欢迎光临药品店！悬壶济世，治病救人是我们最高的宗旨。 客官看着不像村子里的人啊，您是来旅游的吧？恩，这是弊店的见面礼，还望收下，在必要的时候它会派上用场的。"
...
x970007_g_UnDoneMissionInfo={"你确定已经去过药品店了吗？快去吧，药品在这儿是很重要的东西。"}  
--总的大体任务目标描述，一句话描述
x970007_g_MissionAim="去药店取救命的药"		
--任务目标类型(可以多任务目标)


--DELIVERY						--送信                        --npc为要送信给的对象,item为是否有任务物品要给送信对象,该item由任务发布人直接给予玩家; info为任务目标显示
x970007_g_DELIVERY = {{npc = "药品店掌柜",item = 0,num = 0,comeitem = 0,comenum = 0,info="去药店取救命的药",dialog="欢迎光临药品店！悬壶济世，治病救人是我们最高的宗旨。 客观看着不像村子里的人啊，您是来旅游的吧？恩，这是弊店的见面礼，还望收下，在必要的时候它会派上用场的。",type="DELIVERY",order=0}}
...
--经验奖励
x970007_g_ExpBonustxt = "经验  1000+等级×等级"
x970007_g_exp = 1
...
function x970007_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
    ...
    if IsHaveMission(sceneId,selfId,x970007_g_MissionId) > 0 then
		if (GetName(sceneId,targetId)==x970007_g_NameEnd) then
			local m = 0
			for i, QuestInfo in pairs(x970007_g_Quest) do
				if (QuestInfo.type == "DELIVERY") then
					if (#(x970007_g_DELIVERY)==1) and (x970007_g_NameEnd == x970007_g_DELIVERY[1].npc) then	
						m = 2
					else
						m = 1
					end
			...
			elseif m == 2 then
             ...
             AddText(sceneId,"#Y"..x970007_g_MissionName)
		         ...
		         if x970007_g_ItemBonus ~= nil then
		          	for i, item in pairs(x970007_g_ItemBonus) do
			        	AddItemBonus( sceneId, item.id, item.num )
			          end
		         end			
		         if x970007_g_RadioItemBonus ~= nil then
	           		for i, item in pairs(x970007_g_RadioItemBonus) do
		...
		elseif (x970007_g_DELIVERY ~= nil) then
			...
			if  n == 1 then
				for i, DeliveryInfo in pairs(x970007_g_DELIVERY) do
					if (GetName(sceneId,targetId)==DeliveryInfo.npc)then
						...
						elseif m == 0 then
							BeginEvent(sceneId)
								AddText(sceneId,x970007_g_Name.."让你带给我的东西在哪呢？")
							EndEvent( )	
							DispatchEventList(sceneId,selfId,targetId)
						end
					end
...
function	x970007_ShowQuestInfo( sceneId, selfId, targetId ,Done)
	...
	AddText(sceneId,"#Y"..x970007_g_MissionName)
	if DoneEX==2 then
		for i, Info in pairs(x970007_g_ContinueInfo) do
			AddText(sceneId,Info)
		end
	else
		for i, Info in pairs(x970007_g_MissionInfo) do
			AddText(sceneId,Info)
		end
	end
	...
	if (GetName(sceneId,targetId)==x970007_g_Name) then
		if (GetLevel( sceneId, selfId ) >= x970007_g_MissionMinLevel ) and (GetLevel( sceneId, selfId ) <= x970007_g_MissionMaxLevel ) then
			if	x970007_g_MisIdPre == nil then
				...
				for i, questpre in pairs(x970007_g_MisIdPre) do
...
function x970007_OnAccept(sceneId, selfId )		
	...
	for i, QuestInfo in pairs(x970007_g_Quest) do
		if (QuestInfo.type == "DELIVERY") then
				...
				local m = 0
				local ret = 1
				for i, QuestLableInfo in pairs(x970007_g_Quest) do
	...
	AddMission( sceneId,selfId, x970007_g_MissionId, x970007_g_ScriptId, DoKill, DoArea, DoItem )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x970007_g_MissionId)			--得到任务的序列号
	...
	BeginEvent(sceneId)
	  	strText = "你接受了任务  "..x970007_g_MissionName
	  	AddText(sceneId,strText);
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
end
...
function x970007_OnSubmit( sceneId, selfId, targetId,selectRadioId )
	...
	if (x970007_CheckSubmit( sceneId, selfId ) == 1) then
		...
		local givebonus = 0
		local giveitem = 0
		if (#(x970007_g_ItemBonus) ==0)  and  (#(x970007_g_RadioItemBonus) == 0 ) then
			givebonus = 1
		else
			for i, item in pairs(x970007_g_ItemBonus) do
		...
		if (giveitem<=0) and (givebonus==0)  then
			...
			if ret > 0 then
				for i, QuestInfo in pairs(x970007_g_Quest) do
					if QuestInfo.type ==  "DELIVERY" then
						if #(x970007_g_DELIVERY) == 1 and QuestInfo == x970007_g_DELIVERY[1] then
				...
				BeginEvent(sceneId)
				  	strText = "你完成了任务  "..x970007_g_MissionName
				  	AddText(sceneId,strText);
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,selfId)
