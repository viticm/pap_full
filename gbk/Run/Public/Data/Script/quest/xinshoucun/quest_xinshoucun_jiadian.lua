...
x970001_g_DELIVERY = {{npc = "多隆",item = 0,num = 0,comeitem = 0,comenum = 0,info="",dialog="",type="DELIVERY",order=0}}
...
--经验奖励
x970001_g_ExpBonustxt = "经验  1000+等级×等级"
x970001_g_exp = 1
...
function x970001_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
    ...
    if IsHaveMission(sceneId,selfId,x970001_g_MissionId) > 0 then
		if (GetName(sceneId,targetId)==x970001_g_NameEnd) then
			local m = 0
			for i, QuestInfo in pairs(x970001_g_Quest) do
				if (QuestInfo.type == "DELIVERY") then
					if (#(x970001_g_DELIVERY)==1) and (x970001_g_NameEnd == x970001_g_DELIVERY[1].npc) then	
						m = 2
					else
						m = 1
					end
		...
		elseif (x970001_g_DELIVERY ~= nil) then
			...
			if  n == 1 then
				for i, DeliveryInfo in pairs(x970001_g_DELIVERY) do
					if (GetName(sceneId,targetId)==DeliveryInfo.npc)then
						...
						elseif m == 0 then
							BeginEvent(sceneId)
								AddText(sceneId,x970001_g_Name.."让你带给我的东西在哪呢？")
							EndEvent( )	
							DispatchEventList(sceneId,selfId,targetId)
						end
					end
...
function	x970001_ShowQuestInfo( sceneId, selfId, targetId ,Done)
	...
	if DoneEX==2 then
		for i, Info in pairs(x970001_g_ContinueInfo) do
			AddText(sceneId,Info)
		end
	else
		for i, Info in pairs(x970001_g_MissionInfo) do
			AddText(sceneId,Info)
		end
	end
	...
	if (GetName(sceneId,targetId)==x970001_g_Name) then
		if (GetLevel( sceneId, selfId ) >= x970001_g_MissionMinLevel ) and (GetLevel( sceneId, selfId ) <= x970001_g_MissionMaxLevel ) then
			if	x970001_g_MisIdPre == nil then
				...
				for i, questpre in pairs(x970001_g_MisIdPre) do
...
function x970001_OnAccept(sceneId, selfId )		
	...
	for i, QuestInfo in pairs(x970001_g_Quest) do
		if (QuestInfo.type == "DELIVERY") then
				...
				local m = 0
				local ret = 1
				for i, QuestLableInfo in pairs(x970001_g_Quest) do
	...
	AddMission( sceneId,selfId, x970001_g_MissionId, x970001_g_ScriptId, DoKill, DoArea, DoItem )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x970001_g_MissionId)			--得到任务的序列号
	...
	BeginEvent(sceneId)
	  	strText = "你接受了任务  "..x970001_g_MissionName
	  	AddText(sceneId,strText);
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
end
...
function x970001_OnSubmit( sceneId, selfId, targetId,selectRadioId )
	...
	if (x970001_CheckSubmit( sceneId, selfId ) == 1) then
			...
			if ret > 0 then
				for i, QuestInfo in pairs(x970001_g_Quest) do
					if QuestInfo.type ==  "DELIVERY" then
						if #(x970001_g_DELIVERY) == 1 and QuestInfo == x970001_g_DELIVERY[1] then
				...
				BeginEvent(sceneId)
				  	strText = "你完成了任务  "..x970001_g_MissionName
				  	AddText(sceneId,strText);
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,selfId)
