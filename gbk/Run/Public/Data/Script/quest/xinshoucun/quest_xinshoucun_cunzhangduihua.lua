...
--任务是否已经完成
x970002_g_IsMissionOkFail = 0		--变量的第0位

x970002_g_DemandItem ={}

--任务名称
x970002_g_MissionName="村长对话"
--任务描述(接任务时，和所有目标完成交任务时)支持多条语句
x970002_g_MissionInfo={"咱们的南边有一个小村子，拿着我的介绍信，尝试与古代人接触接触吧！看看你们能否融洽。不用担心，他们都是很淳朴的农民。"} 

x970002_g_MissionTarget="找到#aB{112,138,3,杨柳村,村长}[村长]#aE，并与#aB{112,138,3,杨柳村,村长}[村长]#aE对话"	

--任务未完成时去交任务的提示文字 ，支持多条语句
x970002_g_UnDoneMissionInfo={"速去速回"}

x970002_g_MissionComplete="你好啊！年轻人，欢迎你来到我们的村子。我是这个村子的村长，如果有什么不明的事情都可以来问我。"
...
--任务描述(接任务时，和所有目标完成交任务时)支持多条语句
--x970002_g_UnDoneMissionInfo={"已经拿到衣服了？怎么没见你穿呢？？？"}  
--总的大体任务目标描述，一句话描述
x970002_g_MissionAim="去时装店取时髦的衣服"		
--任务目标类型(可以多任务目标)


--DELIVERY						--送信                        --npc为要送信给的对象,item为是否有任务物品要给送信对象,该item由任务发布人直接给予玩家; info为任务目标显示
x970002_g_DELIVERY = {{npc = "村长",item = 13010085,num = 1,comeitem = 0,comenum = 0,info="",dialog="。",type="DELIVERY",order=0}}
...
--经验奖励
x970002_g_ExpBonustxt = "经验  1000+等级×等级"
x970002_g_exp = 1
...
function x970002_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
    ...
    if IsHaveMission(sceneId,selfId,x970002_g_MissionId) > 0 then
		if (GetName(sceneId,targetId)==x970002_g_NameEnd) then
			local m = 0
			for i, QuestInfo in pairs(x970002_g_Quest) do
				if (QuestInfo.type == "DELIVERY") then
					if (#(x970002_g_DELIVERY)==1) and (x970002_g_NameEnd == x970002_g_DELIVERY[1].npc) then	
						m = 2
					else
						m = 1
					end
		...
		elseif (x970002_g_DELIVERY ~= nil) then
			...
			if  n == 1 then
				for i, DeliveryInfo in pairs(x970002_g_DELIVERY) do
					if (GetName(sceneId,targetId)==DeliveryInfo.npc)then
						...
						elseif m == 0 then
							BeginEvent(sceneId)
								AddText(sceneId,x970002_g_Name.."让你带给我的东西在哪呢？")
							EndEvent( )	
							DispatchEventList(sceneId,selfId,targetId)
						end
					end
...
function	x970002_ShowQuestInfo( sceneId, selfId, targetId ,Done)
	...
	AddText(sceneId,"#Y"..x970002_g_MissionName)
	if DoneEX==2 then
		for i, Info in pairs(x970002_g_ContinueInfo) do
			AddText(sceneId,Info)
		end
	else
		for i, Info in pairs(x970002_g_MissionInfo) do
			AddText(sceneId,Info)
		end
	end
	...
	if (GetName(sceneId,targetId)==x970002_g_Name) then
		if (GetLevel( sceneId, selfId ) >= x970002_g_MissionMinLevel ) and (GetLevel( sceneId, selfId ) <= x970002_g_MissionMaxLevel ) then
			if	x970002_g_MisIdPre == nil then
				...
				for i, questpre in pairs(x970002_g_MisIdPre) do
...
function x970002_OnAccept(sceneId, selfId )		
	...
	for i, QuestInfo in pairs(x970002_g_Quest) do
		if (QuestInfo.type == "DELIVERY") then
				...
				local m = 0
				local ret = 1
				for i, QuestLableInfo in pairs(x970002_g_Quest) do
	...
	AddMission( sceneId,selfId, x970002_g_MissionId, x970002_g_ScriptId, DoKill, DoArea, DoItem )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x970002_g_MissionId)			--得到任务的序列号
	...
	BeginEvent(sceneId)
	  	strText = "你接受了任务  "..x970002_g_MissionName
	  	AddText(sceneId,strText);
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
end
...
function x970002_OnSubmit( sceneId, selfId, targetId,selectRadioId )
	...
	if (x970002_CheckSubmit( sceneId, selfId ) == 1) then
			...
			if ret > 0 then
				for i, QuestInfo in pairs(x970002_g_Quest) do
					if QuestInfo.type ==  "DELIVERY" then
						if #(x970002_g_DELIVERY) == 1 and QuestInfo == x970002_g_DELIVERY[1] then
				...
				BeginEvent(sceneId)
				  	strText = "你完成了任务  "..x970002_g_MissionName
				  	AddText(sceneId,strText);
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,selfId)
