...
--任务是否已经完成
x300305_g_IsMissionOkFail = 0		--变量的第0位

x300305_g_DemandKill ={}

--任务名称
x300305_g_MissionName="抢回剩余粮食"
--任务描述(接任务时，和所有目标完成交任务时)支持多条语句
x300305_g_MissionInfo={"抢匪们把抢走的粮食藏了起来，杀死他们，夺回被抢走的粮食！"} 

x300305_g_MissionTarget="到地下粮仓杀死抢粮坏蛋，带回100袋被抢走的粮食"	

x300305_g_ContinueInfo={"快把足够的粮食带回来！"}  

x300305_g_MissionComplete="谢谢你，英雄！有了这些粮食，大汗征战必定凯旋而归！"
...
x300305_g_COLLECT = {{item=13010081,name="被盗粮食",num=100,type="COLLECT",order=0}}
...
--经验奖励
x300305_g_ExpBonustxt = "经验  1000+等级×等级"
x300305_g_exp = 1
...
function x300305_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
    ...
    if IsHaveMission(sceneId,selfId,x300305_g_MissionId) > 0 then
		if (GetName(sceneId,targetId)==x300305_g_NameEnd) then
			local m = 0
			for i, QuestInfo in pairs(x300305_g_Quest) do
				if (QuestInfo.type == "DELIVERY") then
					if (#(x300305_g_DELIVERY)==1) and (x300305_g_NameEnd == x300305_g_DELIVERY[1].npc) then	
						m = 2
					else
						m = 1
					end
		...
		elseif (x300305_g_DELIVERY ~= nil) then
			...
			if  n == 1 then
				for i, DeliveryInfo in pairs(x300305_g_DELIVERY) do
					if (GetName(sceneId,targetId)==DeliveryInfo.npc)then
						...
						elseif m == 0 then
							BeginEvent(sceneId)
								AddText(sceneId,x300305_g_Name.."让你带给我的东西在哪呢？")
							EndEvent( )	
							DispatchEventList(sceneId,selfId,targetId)
						end
					end
...
function	x300305_ShowQuestInfo( sceneId, selfId, targetId ,Done)
	...
	if DoneEX==2 then
		for i, Info in pairs(x300305_g_ContinueInfo) do
			AddText(sceneId,Info)
		end
	else
		for i, Info in pairs(x300305_g_MissionInfo) do
			AddText(sceneId,Info)
		end
	end
	...
	if DoneEX == 2 then
		...
		if x300305_g_ItemBonus ~= nil then
			for i, item in pairs(x300305_g_ItemBonus) do
				AddItemBonus( sceneId, item.id, item.num )
			end
		end			
		if x300305_g_RadioItemBonus ~= nil then
			for i, item in pairs(x300305_g_RadioItemBonus) do
	...
	if (GetName(sceneId,targetId)==x300305_g_Name) then
		if (GetLevel( sceneId, selfId ) >= x300305_g_MissionMinLevel ) and (GetLevel( sceneId, selfId ) <= x300305_g_MissionMaxLevel ) then
			if	x300305_g_MisIdPre == nil then
				...
				for i, questpre in pairs(x300305_g_MisIdPre) do
...
function x300305_OnAccept(sceneId, selfId )		
	...
	AddMission( sceneId,selfId, x300305_g_MissionId, x300305_g_ScriptId, DoKill, DoArea, DoItem )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x300305_g_MissionId)			--得到任务的序列号
	...
	BeginEvent(sceneId)
	  	strText = "你接受了任务  "..x300305_g_MissionName
	  	AddText(sceneId,strText);
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
end
...
function x300305_OnScneneTimer(sceneId, selfId)
	...
	if x300305_g_PROTECTINFO.Step == 3   then
		...
		for ii=0, nMonsterNum-1 do
			...
			for j, ProtectInfo in pairs(x300305_g_PROTECT) do
		...
		if bHaveMonster == #(x300305_g_PROTECT)  then
			for j, ProtectInfo in pairs(x300305_g_PROTECT) do
	...
	if x300305_g_PROTECTINFO.Step == 4   then
		...
		for j, NpcID in pairs(x300305_g_ProtectNPCID) do 
			...
			for i=1, x300305_g_PROTECTINFO.PlayerNum  do
				if x300305_g_PROTECTINFO.PlayerId[i] ~= 0  then
					...
					if distance1 > 20    then
						--提示这个玩家任务失败
						BeginEvent(sceneId)
							strText = x300305_g_MissionName.."  失败"
							AddText(sceneId,strText);
						EndEvent(sceneId)
		...
		if (bDone >= #(x300305_g_PROTECT)) then
...
function x300305_CloseTimer( sceneId, TimerIndex )
	...
	for i =1, #(x300305_g_PROTECTINFO.PlayerId) do
		x300305_g_PROTECTINFO.PlayerId[i]=0
	end
...
function x300305_OnSubmit( sceneId, selfId, targetId,selectRadioId )
	...
	if (x300305_CheckSubmit( sceneId, selfId ) == 1) then
		...
		local givebonus = 0
		local giveitem = 0
		if (#(x300305_g_ItemBonus) ==0)  and  (#(x300305_g_RadioItemBonus) == 0 ) then
			givebonus = 1
		else
			for i, item in pairs(x300305_g_ItemBonus) do
		...
		if (giveitem<=0) and (givebonus==0)  then
			...
			if ret > 0 then
				...
				BeginEvent(sceneId)
				  	strText = "你完成了任务  "..x300305_g_MissionName
				  	AddText(sceneId,strText);
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,selfId)
				if (x300305_g_ScriptIdNext.ScriptId~=nil) and (x300305_g_ScriptIdNext.ScriptId>0) then
					...
					if (CanNext == 1) then
						...
						BeginEvent(sceneId)
							AddText(sceneId,"等你到了"..x300305_g_ScriptIdNext.LV.."级再来找我，我还有很重要的事要交给你去做。千万记住了！")
						EndEvent( )	
						DispatchEventList(sceneId,selfId,targetId)
					end
				end
