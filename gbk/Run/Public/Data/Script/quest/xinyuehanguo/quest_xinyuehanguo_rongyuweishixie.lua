...
x211055_g_MissionName="荣誉卫士鞋"
x211055_g_MissionInfo="    我是大都派来的使者，运送贡品来到贵国。途经野外不幸被土匪劫道，#c0080C0使者密令#W全部丢失。因为没有了这些信，我的车队堵在城外无法进城。如果你能帮我抢回来一些，我将把这把我从大都带过来的精良装备送给你。"
x211055_g_MissionTarget="    收集10个#c0080C0使者密令#W并带给#G阿甘#W"		
x211055_g_MissionComplete="    没有使者密令我的任务就无法完成，拜托你了。"					--完成任务npc说话的话
x211055_g_ContinueInfo="    有了这些使者密令我的损失少了很多，谢谢你。"
--任务奖励
...
function x211055_OnDefaultEvent(sceneId, selfId, targetId)	--点击该任务后执行此脚本
	...
	if IsHaveMission(sceneId,selfId, x211055_g_MissionId) > 0 then
		if x211055_CheckSubmit(sceneId, selfId, targetId) <= 0 then
			...
			AddText(sceneId,format("\n    使者密令   %d/%d", haveitem,x211055_g_DemandItem[1].num ))
			EndEvent()
			DispatchEventList(sceneId, selfId, targetId)
		     ...
		     --for i, item in pairs(x211055_g_DemandItem) do  
		     --AddItemBonus(sceneId, item.id, item.num)
		     --end
		     AddMoneyBonus(sceneId, x211055_g_MoneyBonus)
		     for i, item in pairs(x211055_g_ItemBonus) do
	             	AddItemBonus(sceneId, item.id, item.num)
	             end
		     for i, item in pairs(x211055_g_RadioItemBonus) do
...
function x211055_OnAccept(sceneId, selfId)
			...
			AddText(sceneId,"接受任务："..x211055_g_MissionName) 
			EndEvent()
...
function x211055_OnItemChanged(sceneId, selfId, itemdataId)
	if itemdataId == x211055_g_DemandItem[1].id then
		...
		if LuaFnGetItemCount(sceneId,selfId,x211055_g_DemandItem[1].id) <= x211055_g_DemandItem[1].num then
			haveitem = LuaFnGetItemCount(sceneId,selfId,x211055_g_DemandItem[1].id)
			BeginEvent(sceneId)
			AddText(sceneId,format("使者密令   %d/%d", haveitem,x211055_g_DemandItem[1].num )) 
			EndEvent()
			DispatchMissionTips(sceneId, selfId)
		end
	end
