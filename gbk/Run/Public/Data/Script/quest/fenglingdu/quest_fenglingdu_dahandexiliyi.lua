...
x212009_g_MissionName="大汗的洗礼一"
x212009_g_MissionInfo="    "  --任务描述
x212009_g_MissionInfo2="，大汗被仇恨的恶魔所困，无法自拔，我需要你帮他一把，杀了在#G风陵渔村#W指挥战斗的西夏名将#R赵德任#W，熄灭大汗仇恨的怒火。"
x212009_g_MissionTarget="    #G丘处机#W为了要灭大汗的怒火，要你杀了在#G风陵渔村#W指挥战斗的西夏名将#R赵德任#W。"		                                                                                               
x212009_g_MissionComplete="    多谢，我要为大汗洗礼了。"					--完成任务npc说话的话
x212009_g_ContinueInfo="    杀一人而救百人，这便是行善！你杀了赵德任了吗？"
--任务奖励
...
function x212009_OnDefaultEvent(sceneId, selfId, targetId)	--点击该任务后执行此脚本
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x212009_g_MissionId)
		if x212009_CheckSubmit(sceneId, selfId, targetId) <= 0 then
			...
			AddText(sceneId,format("\n    杀死赵德任   %d/%d", GetMissionParam(sceneId,selfId,misIndex,0),x212009_g_DemandKill[1].num ))
			EndEvent()
...
function x212009_OnAccept(sceneId, selfId)
		...
		AddText(sceneId,"接受任务："..x212009_g_MissionName) 
		EndEvent()
...
function x212009_OnAbandon(sceneId, selfId)
	...
	for i, item in pairs(x212009_g_DemandItem) do
...
function x212009_OnSubmit(sceneId, selfId, targetId, selectRadioId)
	if DelMission(sceneId, selfId, x212009_g_MissionId) > 0 then
		...
		for i, item in pairs(x212009_g_RadioItemBonus) do
	 ...
	 num = GetMissionParam(sceneId,selfId,misIndex,0) 
	 	 if objdataId == x212009_g_DemandKill[1].id then 
       		    if num < x212009_g_DemandKill[1].num then
       		         ...
       		         BeginEvent(sceneId)
			 AddText(sceneId,format("杀死赵德任   %d/%d", GetMissionParam(sceneId,selfId,misIndex,0),x212009_g_DemandKill[1].num )) 
			 EndEvent()
			 DispatchMissionTips(sceneId, selfId)
		    end
       		 end
