...
x204008_g_MissionName="军令如山"
x204008_g_MissionInfo="    每个人都可以有自己的信仰，保持自己祖先的宗教规矩。\n \n    我们来的目的不是要和城市居民打仗，而是要和压迫回教徒的屈出律作战而来。 \n \n    城内的百姓只要不抵抗，蒙古大军不会骚扰百姓，成吉思汗已经下了通缉令，不管是谁，只要能把屈出律的人头献给大汗，他都会得到应有的报仇。    "  --任务描述
x204008_g_MissionInfo2="，上面的话你记好了吗？替我把话传令给在前线指挥作战的#G忽必来#W将军。"
x204008_g_MissionTarget="    把哲别的话告诉在前线指挥作战的#G忽必来#W将军。"		
x204008_g_MissionComplete="    哲别将军的话我已经记住了，我会照做。"					--完成任务npc说话的话
--x204008_g_ContinueInfo="    不管付出多大代价，也要把那件东西给拿回来。"
--任务奖励
...
function x204008_OnDefaultEvent(sceneId, selfId, targetId)	--点击该任务后执行此脚本
...
function x204008_OnAccept(sceneId, selfId)
	...
	AddText(sceneId,"接受任务："..x204008_g_MissionName) 
	EndEvent()
...
function x204008_OnAbandon(sceneId, selfId)
	...
	for i, item in pairs(x204008_g_DemandItem) do
...
function x204008_OnSubmit(sceneId, selfId, targetId, selectRadioId)
	...
	if DelMission(sceneId, selfId, x204008_g_MissionId) > 0 then
		...
		for i, item in pairs(x204008_g_RadioItemBonus) do
