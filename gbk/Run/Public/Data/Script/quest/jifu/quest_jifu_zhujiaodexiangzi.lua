	...
	if IsHaveMission(sceneId,selfId, x225002_g_MissionId) > 0 then
			if x225002_CheckSubmit(sceneId, selfId, targetId) <= 0 then
				...
				DispatchEventList(sceneId, selfId, targetId)
			     ...
			     for i, item in pairs(x225002_g_ItemBonus) do
		             	AddItemBonus(sceneId, item.id, item.num)
		             end
			     for i, item in pairs(x225002_g_RadioItemBonus) do
...
function x225002_OnAbandon(sceneId, selfId)
	...
	for i, item in pairs(x225002_g_DemandItem) do
		DelItem(sceneId, selfId, item.id, item.num)
	end
	for i, item in pairs(x225002_g_noDemandItem) do
		DelItem(sceneId, selfId, item.id, item.num)
	end
