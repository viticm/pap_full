...
--任务起始npc
x100016_g_NameHead	= "楚留香"

--任务提交npc
x100016_g_NameEnd	= "楚留香"

--任务是否已经完成
x100016_g_IsMissionOkFail = 0		--变量的第0位

x100016_g_DemandItem ={{id=10051001,num=1}}

x100016_g_MissionName="购买兵器"

--任务描述(接任务时，和所有目标完成交任务时)支持多条语句
x100016_g_MissionInfo={"现在正是非常时期，我们要随时小心黑衣人的袭击，我现在要存下一些装备，以备不时只需。你能帮我吗？"} 

x100016_g_MissionTarget="收集一柄青冥剑，你可以到#aB{302,210,9,苏州01,武器店掌柜}[武器店掌柜]#aE处购买一柄青冥剑，完成后回#aB{333,275,9,苏州01,楚留香}[楚留香]#aE处复命"	

x100016_g_ContinueInfo={"一定要快啊！以备不测。"}

x100016_g_MissionComplete={"这么快就回来了？你果然有做大侠的天赋。"}
...
x100016_g_UnDoneMissionInfo={"找到武器商人，并购买一柄青冥剑"}
--总的大体任务目标描述，一句话描述
x100016_g_MissionAim="找到武器商人，并购买一柄青冥剑"		

--任务目标类型(可以多任务目标)
--COLLECT						--收集物品                    --item为采集的物品类型,name为道具名称,num为需求道具个数
x100016_g_COLLECT = {{item=10051001,name="青冥剑",num=1,type="COLLECT",order=0}}
...
function x100016_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
    ...
    if IsHaveMission(sceneId,selfId,x100016_g_MissionId) > 0 then
		if (GetName(sceneId,targetId)==x100016_g_NameEnd) then
			local m = 0
			for i, QuestInfo in x100016_g_Quest do
			...
			if m == 1 then
             ...
             AddText(sceneId,"#Y"..x100016_g_MissionName)
		         ...
		         if x100016_g_ItemBonus ~= nil then
		          	for i, item in x100016_g_ItemBonus do
		...
		elseif (x100016_g_DELIVERY ~= nil) then
			...
			if  n == 1 then
				for i, DeliveryInfo in x100016_g_DELIVERY do
					if (GetName(sceneId,targetId)==DeliveryInfo.npc)then
						...
						elseif m == 0 then
							BeginEvent(sceneId)
								AddText(sceneId,x100016_g_NameHead.."让你带给我的东西在哪呢？")
							EndEvent( )	
							DispatchEventList(sceneId,selfId,targetId)
						end
					end
...
function	x100016_ShowQuestInfo( sceneId, selfId, targetId ,Done)
	...
	AddText(sceneId,"#Y"..x100016_g_MissionName)
	if DoneEX==2 then
		for i, Info in x100016_g_UnDoneMissionInfo do
			AddText(sceneId,Info)
		end
	else
		for i, Info in x100016_g_MissionInfo do
			AddText(sceneId,Info)
		end
	end
	...
	if (GetName(sceneId,targetId)==x100016_g_NameHead) then
		if (GetLevel( sceneId, selfId ) >= x100016_g_MissionMinLevel ) and (GetLevel( sceneId, selfId ) <= x100016_g_MissionMaxLevel ) then
			if	x100016_g_MisIdPre == nil then
				...
				for i, questpre in x100016_g_MisIdPre do
...
function x100016_OnAccept(sceneId, selfId )		
	...
	AddMission( sceneId,selfId, x100016_g_MissionId, x100016_g_ScriptId, DoKill, DoArea, DoItem )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x100016_g_MissionId)			--得到任务的序列号
	...
	BeginEvent(sceneId)
	  	strText = "你接受了任务  "..x100016_g_MissionName
	  	AddText(sceneId,strText);
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
end
		...
		if (giveitem<=0) and (givebonus==0)  then
			...
			if ret > 0 then
			 for i, QuestInfo in x100016_g_Quest do					
				...
				BeginEvent(sceneId)
				  	strText = "你完成了任务  "..x100016_g_MissionName
				  	AddText(sceneId,strText);
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,selfId)
