...
--任务是否已经完成
x110001_g_IsMissionOkFail = 0		--变量的第0位

x110001_g_DemandItem ={}

--任务名称
x110001_g_MissionName="南湖寻人"
--任务描述(接任务时，和所有目标完成交任务时)支持多条语句
x110001_g_MissionInfo={"江南的英雄众多，尽量通知一下，小心黑衣人的毒手。"} 

x110001_g_MissionTarget="通知柯镇恶，他在南湖野外。你可以通过我身边的NPC传送到南湖。在南湖野外找到#aB{22,35,10,南湖野外,柯镇恶}[柯镇恶]#aE"	

--任务未完成时去交任务的提示文字 ，支持多条语句
x110001_g_UnDoneMissionInfo={"快去快回吧"}

x110001_g_MissionComplete="多谢小兄弟，也带我感谢楚大侠。小老儿会小心的。"
...
--任务描述(接任务时，和所有目标完成交任务时)支持多条语句
--x110001_g_UnDoneMissionInfo={"去找楚留香吧！"}  
--总的大体任务目标描述，一句话描述
x110001_g_MissionAim="与楚留香谈话"		
--任务目标类型(可以多任务目标)


--DELIVERY						--送信                        --npc为要送信给的对象,item为是否有任务物品要给送信对象,该item由任务发布人直接给予玩家; info为任务目标显示
x110001_g_DELIVERY = {{npc = "柯镇恶",item = 0,num = 0,comeitem = 0,comenum = 0,info="",dialog="",type="DELIVERY",order=0}}
...
--经验奖励
x110001_g_ExpBonustxt = "经验  1000+等级×等级"
x110001_g_exp = 1
...
function x110001_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
    ...
    if IsHaveMission(sceneId,selfId,x110001_g_MissionId) > 0 then
		if (GetName(sceneId,targetId)==x110001_g_NameEnd) then
			local m = 0
			for i, QuestInfo in x110001_g_Quest do
				if (QuestInfo.type == "DELIVERY") then
					if (getn(x110001_g_DELIVERY)==1) and (x110001_g_NameEnd == x110001_g_DELIVERY[1].npc) then	
						m = 2
					else
						m = 1
					end
		...
		elseif (x110001_g_DELIVERY ~= nil) then
			...
			if  n == 1 then
				for i, DeliveryInfo in x110001_g_DELIVERY do
					if (GetName(sceneId,targetId)==DeliveryInfo.npc)then
						...
						elseif m == 0 then
							BeginEvent(sceneId)
								AddText(sceneId,x110001_g_Name.."让你带给我的东西在哪呢？")
							EndEvent( )	
							DispatchEventList(sceneId,selfId,targetId)
						end
					end
...
function	x110001_ShowQuestInfo( sceneId, selfId, targetId ,Done)
	...
	AddText(sceneId,"#Y"..x110001_g_MissionName)
	if DoneEX==2 then
		for i, Info in x110001_g_ContinueInfo do
			AddText(sceneId,Info)
		end
	else
		for i, Info in x110001_g_MissionInfo do
			AddText(sceneId,Info)
		end
	end
	...
	if (GetName(sceneId,targetId)==x110001_g_Name) then
		if (GetLevel( sceneId, selfId ) >= x110001_g_MissionMinLevel ) and (GetLevel( sceneId, selfId ) <= x110001_g_MissionMaxLevel ) then
			if	x110001_g_MisIdPre == nil then
				...
				for i, questpre in x110001_g_MisIdPre do
...
function x110001_OnAccept(sceneId, selfId )		
	...
	for i, QuestInfo in x110001_g_Quest do
		if (QuestInfo.type == "DELIVERY") then
				...
				local m = 0
				local ret = 1
				for i, QuestLableInfo in x110001_g_Quest do
	...
	AddMission( sceneId,selfId, x110001_g_MissionId, x110001_g_ScriptId, DoKill, DoArea, DoItem )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x110001_g_MissionId)			--得到任务的序列号
	...
	BeginEvent(sceneId)
	  	strText = "你接受了任务  "..x110001_g_MissionName
	  	AddText(sceneId,strText);
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
end
...
function x110001_OnSubmit( sceneId, selfId, targetId,selectRadioId )
	...
	if (x110001_CheckSubmit( sceneId, selfId ) == 1) then
			...
			if ret > 0 then
				for i, QuestInfo in x110001_g_Quest do
					if QuestInfo.type ==  "DELIVERY" then
						if getn(x110001_g_DELIVERY) == 1 and QuestInfo == x110001_g_DELIVERY[1] then
				...
				BeginEvent(sceneId)
				  	strText = "你完成了任务  "..x110001_g_MissionName
				  	AddText(sceneId,strText);
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,selfId)
