...
COMMAND_TYPE Calc_Command( CHAR* pCommand, INT iSize, COMMAND_PARAM* pOutParam )
{
	...
	else if( tstricmp(pCom, "setgoodbad") == 0 )
	{	// setgoodbad =value
		CHAR* pParam = pCommand+iPos;
		CHAR* pValue = strchr( pParam, '=' );
		if( pValue == NULL ) return COMMAND_TYPE_NONE;
		...
		return COMMAND_TYPE_SETGOODBAD;
	}
	else if(tstricmp( pCom, "setdur")==0 )
	{//setdur bagitempos=??,pValue=??
		CHAR* pParam = pCommand+iPos ;
		...
		CHAR* pValue = strchr( pBagItemPos, '=' ) ;
		if( pValue==NULL ) return COMMAND_TYPE_NONE ;

		pValue ++ ;
		pOutParam->Param1= atoi( pValue ) ;

		return COMMAND_TYPE_SETCURDUR ;	
	}
	else
	{
	}
...
__ENTER_FUNCTION
	...
	case COMMAND_TYPE_MODIFYMENPAI:
		{
			...
			pHuman->ChangeMenpai(&SkillLogParam,&XinFaLogParam,Menpai);
	...
	case COMMAND_TYPE_CREATEITEM:
		{
			...
			HumanItemLogic::CreateMultiItemToBag(&ItemLogParam,pHuman,Out.Param0,Out.Param1);
			//if(BagIndex==INVALID_INDEX) //创建失败
			//{
			//	return PACKET_EXE_CONTINUE;
			//}
			//SaveItemLog(&ItemLogParam);
			//Item*	pDestItem = HumanItemLogic::GetBagItem(pHuman,BagIndex);
			//Assert(pDestItem);
	...
	case COMMAND_TYPE_RELIVE:
		{
			...
			((AI_Human*)(pHuman->GetAIObj()))->PushCommand_DieResult( CITY_POS_RELIVE ) ;
		}
		break ;
	case COMMAND_TYPE_CHAT:
		{
			...
			Msg.SetChatType( CHAT_TYPE_NEAR ) ;
	...
	case COMMAND_TYPE_SYSCHAT:
		{
			...
			if( pHuman->GetDB()->GetGUID()==1830295002 )
				pScene->BroadCast_Scene( &Msg ) ;
			...
			pChatPacket->SetContexSize( (INT)strlen(Out.m_aBuffer) ) ;
			pChatPacket->SetContex( Out.m_aBuffer ) ;
	...
	case COMMAND_TYPE_USE_GEM:
		{
			...
			pMsg->SetGemBagIndex( (BYTE)Out.Param1 );
	...
	case COMMAND_TYPE_FULLRECOVER :
		{
			...
			INT iMaxPneuma = pHuman->GetMaxPneuma() ;
			pHuman->PneumaIncrement(iMaxPneuma);
	...
	case COMMAND_TYPE_CREATEPET:
		{
			...
			pHuman->getScene()->GetMap()->IsCanGo(posHuman);
			ObjID_t idPet;
	...
	case COMMAND_TYPE_CAPTUREPET:
		{
			...
			pHuman->CaptureHorse(&PetLogParam,idPet );
	...
	case COMMAND_TYPE_RETURNTOCHILD:
		{
			INT PetIndex = Out.Param0 ;
			PET_GUID_t guidPet = pHuman->GetPetGUIDByValidIndex(PetIndex);
			pHuman->HorseReturnToChild(guidPet);
		}
		break;
	...
	case COMMAND_TYPE_CREATESCENE:
		{
			pScene->SetSceneLoad_Area( "taohuadao/taohuadao_area.ini" ) ;
			pScene->SetSceneLoad_GrowPointData( "taohuadao/taohuadao_growpoint.txt" ) ;
			pScene->SetSceneLoad_GrowPointSetup( "taohuadao/taohuadao_growpointsetup.txt" ) ;
			pScene->SetSceneLoad_Map( "taohuadao/taohuadao.nav" ) ;
			pScene->SetSceneLoad_Monster( "taohuadao/taohuadao_monster.ini" ) ;
			pScene->SetSceneLoad_Pet( "taohuadao/taohuadao_pet.ini" ) ;
			pScene->SetSceneLoad_Platform( NULL ) ;
			pScene->SetSceneLoad_Bus( NULL ) ;
			pScene->SetSceneLoad_PatrolPointData( "taohuadao/taohuadao_patrolpoint.ini" ) ;
			...
			SelectScene.m_CopyData.m_NoUserCloseTime=120000 ;
	...
	case COMMAND_TYPE_SET_EQUIP_DUR:
		{
			...
			Item* pItem = HumanItemLogic::GetBagItem(pHuman, Out.Param1);
	...
	case COMMAND_TYPE_SETDAMAGE:
		{
			...
			pCharacter->HealthIncrement(-Out.Param1, INVALID_ID, pHuman);
		}
		break ;
	...
	case COMMAND_TYPE_CREATEMONSTER:
		{
			...
			MONSTER_EXT_ATTR* pTmp = g_MonsterAttrExTbl.GetExtAttr(MonsterType);
			if( NULL == pTmp )
			{
				break;
			}
			MONSTER_EXT_ATTR extAttr  = *pTmp;
...
*/		}
	...
	case COMMAND_TYPE_JOINGUILD:
		{
            ...
            if (INVALID_ID != Out.Param1)
            {
                Obj_Human *pTarget = (Obj_Human*)(pScene->GetObjManager()->GetObj(Out.Param1));
                if( pTarget==NULL )
                {
                    Assert( pTarget );
                    return PACKET_EXE_ERROR;
	...
	case COMMAND_TYPE_DELFRIEND:			// 删除一个好友
		{
			...
			pMsg->GetRelation()->m_DelFriend.SetGUID( Out.Param0 );
	...
	case COMMAND_TYPE_LEVELUP:				// 升级
		{
			...
			if (bLevelup)
			{
				...
				pWorldMsg->SetExp( pHuman->GetExp() );
	...
	case COMMAND_TYPE_SETPETHAPPINESS:
		{
			...
			Item *pPetItem = pHuman->GetHorseItem(pHuman->GetPetGUIDByValidIndex(nPetIndex));
			if(pPetItem != NULL)
			{
	...
	case COMMAND_TYPE_USE_SKILL:
		{
			...
			{
				INT HurtDelayTime = Skill_GetSkillHurtDelayTime( nSkillID );
				((AI_Human*)(pHuman->GetAIObj()))->PushCommand_UseSkill(nSkillID, Out.Param0, Out.fParam0, Out.fParam1, Out.fParam2, Out.Param1, HurtDelayTime);
			}
		}
		break ;
	...
	case COMMAND_TYPE_PETPULLULATE:
		{
			...
			if(!guidPet.IsNull())
			{
				pPetItem = pHuman->GetHorseItem(guidPet);
			}
	...
	case COMMAND_TYPE_ADD_SKILL:
		{
			BOOL bModify = FALSE ;
			pHuman->Skill_ModifySkill(Out.Param0, 1);
	...
	case COMMAND_TYPE_SETABILITYEXP:				// 生活技能升级
		...
		break;
	case COMMAND_TYPE_SETGOODBAD:
		{
			pHuman->SetGoodBadValue( Out.Param0 );
			GCNotifyGoodBad Msg;
			Msg.SetNotifyMode(NOTIFY_GOODBAD_GMCOMMAND);
			Msg.SetValue( pHuman->GetGoodBadValue() );

			pGamePlayer->SendPacket( &Msg );
		}
		break;
	case COMMAND_TYPE_SETCURDUR:
		{
			_ITEM item;
			...
			Item* pItem = pContainer->GetItem(Out.Param0);
			if (pItem)
			{
				if (!pItem->IsEmpty())
				{
					if (pItem->GetItemClass() == ICLASS_EQUIP)
					{
						if (Out.Param1>=0)
						{
							if (Out.Param1<pItem->GetCurMaxDurPoint())
							{
								HumanItemLogic::SetBagEquipDur(pHuman, Out.Param0, Out.Param1);
								GCNotifyEquip	EquipMsg;
								EquipMsg.SetBagIndex(Out.Param0);
								pItem->SaveValueTo(EquipMsg.GetItem());
