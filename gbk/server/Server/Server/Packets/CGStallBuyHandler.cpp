...
uint CGStallBuyHandler::Execute( CGStallBuy* pPacket, Player* pPlayer )
{
	...
	{//买物品
		...
		// 设置物品传输字串，用于客户端显示物品的supertooltip	20100706 BLL
		...
		sprintf(szExchangeMessage,"#{_INFOUSR%s}购买 #{_INFOMSG%s} X %d, 共花费%d金%d银%d铜", pHuman->GetName(), strItemTransfer, MessItemNum, Gold, Silver, copper);
	...
	{//买宠物
		...
		UINT Silver		= (UINT)((CurPrice%10000)/100);
		UINT copper		= (UINT)(CurPrice%100);
		sprintf(szExchangeMessage,"#{_INFOUSR%s}购买 宠物#{_INFOMSG%s}, 共花费%d金%d银%d铜", pHuman->GetName(), pIt->GetName(), Gold, Silver, copper);
		UINT	NewID = pBBS->NewMessageID();
		if(pBBS->AddNewMessageByID(NewID, szExchangeMessage, (INT)strlen(szExchangeMessage), "_SYSTEM") == FALSE)
		{
