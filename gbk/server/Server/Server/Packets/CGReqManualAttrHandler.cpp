...
uint CGReqManualAttrHandler::Execute(CGReqManualAttr* pPacket,Player* pPlayer)
{
	...
	if(FALSE == pHuman->ManualAttrPoint(nStr, nCon, nInt, nDex))
	{
	...
	g_pLog->FastSaveLog( LOG_FILE_1, "CGReqManualAttrHandler: Obj=%d: +Str:  +Con: %d, +Int: %d, +Dex: %d, ", 
		pHuman->GetID(), nStr, nCon, nInt, nDex);
