...
uint CGEquipChangePaiTypeHandler::Execute(CGEquipChangePaiType* pPacket,Player* pPlayer)
{
	__ENTER_FUNCTION
	...
	{
		g_pLog->FastSaveLog( LOG_FILE_1, "CGEquipChangePaiTypeHandler Out Of Range ObjName = %s", pHuman->GetName()) ;
		return PACKET_EXE_CONTINUE ; 
	}


	EQUIPCHANGEPAITTYPE_RESULT	Result = EQUIPCHANGEPAITTYPE_FAIL;
	GCEquipChangePaiTypeResult  ResultPacket;

	Item*		pEquipItem		= NULL;
	INT			EquipPoint		= INVALID_ITEM_POS;
	if(pPacket->GetEquipPosType()>=INVALID_ITEM_POS_TYPE)
	{
		g_pLog->FastSaveLog( LOG_FILE_1, "CGEquipChangePaiTypeHandler Equip posType error, posType=%d", pPacket->GetEquipPosType()) ;
		return PACKET_EXE_ERROR;
	}
	...
	if(pPacket->GetEquipPosType() == ENUM_BAG)
	{
		...
		{
			g_pLog->FastSaveLog( LOG_FILE_1, "CGEquipChangePaiTypeHandler: BagIndex error, BagIndex=%d", BagIndex) ;
			return PACKET_EXE_ERROR;
		}
		if (!pItemContainer->IsCanUse())
		{
			Result = EQUIPCHANGEPAITTYPE_BAG_INVALID;
			ResultPacket.SetResult(Result);
			pGamePlayer->SendPacket(&ResultPacket);
			g_pLog->FastSaveLog( LOG_FILE_1, "CGEquipChangePaiTypeHandler: ItemContainer is invalid, ContainerType=%d, BagIndex=%d",
		...
		{
			Result = EQUIPCHANGEPAITTYPE_EQUIP_FAIL;
			ResultPacket.SetResult(Result);
			pGamePlayer->SendPacket(&ResultPacket);
			g_pLog->FastSaveLog( LOG_FILE_1, "CGEquipChangePaiTypeHandler Equip empyty, bagpos=%d", pPacket->GetPos()) ;
			return PACKET_EXE_CONTINUE;
		}
		...
		if(pEquipItem->GetItemClass()!=ICLASS_EQUIP)
		{
			...
			ResultPacket.SetResult(Result);
			pGamePlayer->SendPacket(&ResultPacket);
			g_pLog->FastSaveLog( LOG_FILE_1, "CGEquipChangePaiTypeHandler is not ICLASS_EQUIP") ;
			return PACKET_EXE_CONTINUE;
		}
	...
	if(pPacket->GetEquipPosType() == ENUM_EQUIP_POINT)
	{
		...
		{
			g_pLog->FastSaveLog( LOG_FILE_1, "CGEquipChangePaiTypeHandler Item pos error") ;
			return PACKET_EXE_ERROR;
		}
		...
		{
			AssertEx(FALSE,"[CGEquipChangePaiTypeHandler]: NULL EquipContainer pointer found!");
		}
	...
	if(!(pEquipItem->GetGUID() == pPacket->getItemID()))
	{
		...
		ResultPacket.SetResult(Result);
		pGamePlayer->SendPacket(&ResultPacket);
		g_pLog->FastSaveLog( LOG_FILE_1, "CGEquipChangePaiTypeHandler guid error") ;
		return PACKET_EXE_CONTINUE;
	}

	Result	= (EQUIPCHANGEPAITTYPE_RESULT)pHuman->EquipChangePai(pEquipItem, pPacket->getGemBagPos());
	ResultPacket.SetResult(Result);
	pGamePlayer->SendPacket(&ResultPacket);
	...
	if((pPacket->GetEquipPosType() == ENUM_EQUIP_POINT)&&(EquipPoint != INVALID_ITEM_POS))
	{
		...
		if(pEquipItem->IsEmpty()) 
		{
			g_pLog->FastSaveLog( LOG_FILE_1, "CGEquipChangePaiTypeHandler: Equip_Point =%d Fail", EquipPoint ) ;
			return PACKET_EXE_CONTINUE;
		}
