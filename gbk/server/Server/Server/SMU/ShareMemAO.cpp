...
BOOL	ShareMemAO::Create(SM_KEY key,uint Size)
{
	__ENTER_FUNCTION
		...
		{
			Log::SaveLog(m_LogPath,"Create ShareMem Error SM_KET = %d",key);
			return FALSE;
		}
		...
		{	
			...
			m_Size	=	Size;
			Log::SaveLog(m_LogPath,"Create ShareMem Ok SM_KET = %d",key);
			return TRUE;
		}
		else
		{
			Log::SaveLog(m_LogPath,"Map ShareMem Error SM_KET = %d",key);
			return FALSE;
		}
	__LEAVE_FUNCTION
		return FALSE ;
...
BOOL	ShareMemAO::Attach(SM_KEY key,uint	Size)
{
	...
	if(m_hold == INVALID_SM_HANDLE)
	{
		...
		Log::SaveLog(m_LogPath,"Attach ShareMem Error SM_KET = %d",key);
		return FALSE;
	}
	...
	if(m_pHeader)
	{
		...
		m_Size	=	Size;
		Log::SaveLog(m_LogPath,"Attach ShareMem OK SM_KET = %d",key);
		return TRUE;
	}
	else
	{
