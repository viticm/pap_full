...
ScanOpt_SObj_ScanChar::ScanOpt_SObj_ScanChar( ):
	m_pTargets(NULL),
	m_fRadius(0.0f),
	m_nCount(0),
	m_pLogic(NULL),
...
BOOL ScanOpt_SObj_ScanChar::Init( SCANOPT_SOBJ_SCANCHAR_INIT* pInit )
{
__ENTER_FUNCTION
	if(pInit==NULL)
	{
		AssertEx(FALSE,"[ScanOpt_SObj_ScanChar::Init]: NULL init struct pointer found!!") ;
		return FALSE ;
	}
	if(NULL==pInit->m_pTargets)
	{
		AssertEx(FALSE,"[ScanOpt_SObj_ScanChar::Init]: NULL Target LIST pointer found!!") ;
		return FALSE ;		
	}
	if(NULL==pInit->m_pMe)
	{
		AssertEx(FALSE,"[ScanOpt_SObj_ScanChar::Init]: NULL me pointer found!!") ;
		return FALSE ;				
	}
	pInit->InitBase();
	...
	if(NULL==m_pLogic)
	{
		AssertEx(FALSE,"[ScanOpt_SObj_ScanChar::Init]: Can not find the logic for this special obj!!") ;
		return FALSE ;				
	}
...
UINT ScanOpt_SObj_ScanChar::On_FindObj(Obj* pObj)
{
__ENTER_FUNCTION
	if(FALSE==m_bReady)
	{
		AssertEx(FALSE,"[ScanOpt_SObj_ScanChar::On_FindObj]:Init first, use second! please~!!");
		return SCANRETURN_RETURN;
	}
	if(pObj==NULL)
	{
		AssertEx(FALSE,"[ScanOpt_SObj_ScanChar::On_FindObj]: NULL Obj Pointer Found!!");
		return SCANRETURN_RETURN;
	}
