...
inline VOID Scene::SetSceneLoad_Bus( const CHAR* szBus )
{
	memset( m_SceneLoad.m_szBus, 0, sizeof(m_SceneLoad.m_szBus) ) ;

	if( szBus==NULL )
	{
		m_SceneLoad.m_IsBusExist = FALSE ;
		return ;
	}

	m_SceneLoad.m_IsBusExist = TRUE ;
	strncpy( m_SceneLoad.m_szBus, szBus, sizeof(m_SceneLoad.m_szBus) ) ;
	m_SceneLoad.m_szBus[sizeof(m_SceneLoad.m_szBus)-1]='\0';
}
...
inline VOID Scene::SetMapName(const CHAR *pszMapName)
{
	if(!pszMapName)
	{
		return;
	}
	strncpy(m_szMapName, pszMapName, sizeof(m_szMapName));
