...
//DestID:本意是表示聊天频道时目的OBJ
//但是在做家族聊天时，发现家族是从属于帮会的，因此仅有家族ID不能唯一标识
//因此在家族聊天时将DestID做为帮会ID，仅此一次下不为例
			...
			case CHAT_TYPE_GUILD:
				{
					...
					init.m_nChatType = pChat->GetChatType() ;
					init.m_pPacket = pChat ;
					init.m_GuildID = (GuildID_t)DestID ;//需要设置帮派ID
			...
			case CHAT_TYPE_JIAZU:
				{
					...
					init.m_JiaZuID = (JiaZuID_t)SourID ;//需要设置帮派ID
			...
			case CHAT_TYPE_COUNTRY:
				{
					...
					init.m_nChatType = pChat->GetChatType() ;
					init.m_pPacket = pChat ;
					init.m_CountryID = (CountryID_t)SourID ;//需要设置帮派ID
