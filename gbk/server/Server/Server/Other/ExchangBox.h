...
#define	EXCHANGE_CERTIFY_ISLOCK(pHuman)\
		...
		{\
			...
			Log::SaveLog( SERVER_LOGFILE, "<交易> [%s] 已锁定交易窗口，操作无效 ", pHuman->GetName() ) ;\
...
#define EXCHANGE_CERTIFY_EACH_OTHER(pHuman)\
		...
		{\
			...
			Log::SaveLog( SERVER_LOGFILE, "<交易> [%s] 已掉线，操作无效 ", pHuman->GetName() ) ;\
			return PACKET_EXE_CONTINUE;\
		}\
		...
		{\
			...
			pHuman->GetPlayer()->SendPacket(&Msg);\
			pHuman->m_ExchangBox.CleanUp();\
			Log::SaveLog( SERVER_LOGFILE, "<交易> [%s] 在与其他人交易，操作无效 ", pTargetHuman->GetName() ) ;\
			return PACKET_EXE_CONTINUE;\
		}\
...
#define	EXCHANGE_CERTIFY_STATUS(pHuman, status)\
		...
		{\
			...
			Log::SaveLog( SERVER_LOGFILE, "<交易> [%s] 交易状态出错 status = %d ", pHuman->GetName(), pHuman->m_ExchangBox.m_Status );\
			return PACKET_EXE_CONTINUE;\
		}\
