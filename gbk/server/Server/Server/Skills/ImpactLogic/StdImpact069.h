/********************************************************************
	created:	2008/05/08
	created:	8:5:2008   15:29
	filename: 	MapServer\Server\Skills\ImpactLogic\StdImpact069.h
	file path:	MapServer\Server\Skills\ImpactLogic
	file base:	StdImpact069
	file ext:	h
	author:		Richard
...
#ifndef StdImpact069_h__
#define StdImpact069_h__
...
namespace Combat_Module
{
	...
	{
		class StdImpact069_T : public ImpactLogic_T
		{
		public:
			enum
			{
			...
			INT GetParaID(OWN_IMPACT const& rImp) const {return rImp.GetParamByIndex(OwnImpactParamIndex_T::IDX_PARA_ID);};
			VOID SetParaID(OWN_IMPACT& rImp, INT const nDamage) const {rImp.SetParamByIndex(OwnImpactParamIndex_T::IDX_PARA_ID, nDamage);};

			INT GetRefixRate(OWN_IMPACT const& rImp) const {return rImp.GetParamByIndex(OwnImpactParamIndex_T::IDX_REFIX_RATE);};
			VOID SetRefixRate(OWN_IMPACT& rImp, INT const nDamage) const {rImp.SetParamByIndex(OwnImpactParamIndex_T::IDX_REFIX_RATE, nDamage);};
