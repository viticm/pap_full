/********************************************************************
	created:	2008/05/08
	created:	8:5:2008   14:55
	filename: 	MapServer\Server\Skills\ImpactLogic\StdImpact068.cpp
...
namespace Combat_Module
{
	...
	{
		BOOL StdImpact068_T::InitFromData(OWN_IMPACT& rImp, ImpactData_T const& rData) const
		{
			__ENTER_FUNCTION
		...
		VOID StdImpact068_T::OnActive(OWN_IMPACT& rImp, Obj_Character& rMe) const
		{
			__ENTER_FUNCTION
		...
		VOID StdImpact068_T::CriticalRefix(OWN_IMPACT& rImp) const
		{
			__ENTER_FUNCTION
		...
		BOOL StdImpact068_T::RefixPowerByRate(OWN_IMPACT & rImp, INT nRate) const
		{
			__ENTER_FUNCTION
			nRate += 100;
		...
		INT StdImpact068_T::GetResult( INT iPara1, FLOAT fPara2, INT iPara3, INT iPara4, INT iRate ) const
		{
			FLOAT fRate = iRate / 1000.0f;
			return (INT)((iPara1*fPara2/1000.0f + iPara3*iPara4/1000.0f) * fRate);
		}

		INT StdImpact068_T::GetRefixPara( OWN_IMPACT & rImp, Obj_Character& rMe ) const
		{
		...
		VOID StdImpact068_T::InitImpact( OWN_IMPACT &rImp ) const
		{
			SetParaID
				(rImp, 
		...
		VOID StdImpact068_T::FinalDamage( INT * vDamage, Obj_Character &rMe ) const
		{
