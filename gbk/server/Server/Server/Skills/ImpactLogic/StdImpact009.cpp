...
namespace Combat_Module
{
	...
	namespace Impact_Module
	{
		...
		{
			__ENTER_FUNCTION
			SetSceneID(rImp, Impact_GetImpactDataDescriptorValueByIndex
			(rImp.GetDataIndex(), 
			ImpactDataDescriptorIndex_T::IDX_SCENE_ID,
			rImp.GetSkillLevel()
			));
			SetPosition_X
				(rImp, 
				Impact_GetImpactDataDescriptorValueByIndex
					(rImp.GetDataIndex(), 
					ImpactDataDescriptorIndex_T::IDX_POSITION_X,
					rImp.GetSkillLevel()
					)+0.f
				);
			SetPosition_Z
				...
				Impact_GetImpactDataDescriptorValueByIndex
					(rImp.GetDataIndex(), 
					ImpactDataDescriptorIndex_T::IDX_POSITION_Z,
					rImp.GetSkillLevel()
					)+0.f
				);
			return TRUE;
