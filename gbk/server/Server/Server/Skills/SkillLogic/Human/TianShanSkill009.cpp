...
namespace Combat_Module
{
	namespace Skill_Module
	{
		...
		//class TianShanSkill009_T : public SkillLogic_T
		//Skill Refix interfaces
		BOOL TianShanSkill009_T::SpecificConditionCheck(Obj_Character& rMe) const
		{
			__ENTER_FUNCTION
		...
		BOOL TianShanSkill009_T::EffectOnUnitOnce(Obj_Character& rMe, Obj_Character& rTar, BOOL bCriticalFlag) const
		{
			__ENTER_FUNCTION
			SkillInfo_T& rSkillInfo = rMe.GetSkillInfo();
			TargetingAndDepletingParams_T& rParams = rMe.GetTargetingAndDepletingParams();
			...
			if(NULL==pGuard)
			{
				AssertEx(FALSE,"[TianShanSkill009_T::TakeEffectNow]:Can't Create Guard. check now.");
