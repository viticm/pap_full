...
protected:
	...
	ORESULT Obj_UseSkill(ObjID_t idSkill, ObjID_t idTarget, FLOAT fTargetX, FLOAT fTargetZ, INT iHurtDelayTime, FLOAT fDir = -1.f, GUID_t guidTarget=INVALID_ID);
...
public: // 必将经过State过滤的操作
	...
	ORESULT Move(INT nHandleID, WORD wNumTargetPos, const WORLD_POS *paTargetPos);
	ORESULT UseSkill(ObjID_t idSkill, ObjID_t idTarget, FLOAT fTargetX, FLOAT fTargetZ, INT iHurtDelayTime, FLOAT fDir = -1.f, GUID_t guidTarget=INVALID_ID);	
