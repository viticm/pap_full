...
public:
	...
	ORESULT Obj_UseSkill(const AI_Character* pAI, ObjID_t idSkill, ObjID_t idTarget, FLOAT fTargetX, FLOAT fTargetZ, INT iHurtDelayTime, FLOAT fDir = -1.f, GUID_t guidTarget=INVALID_ID);
	...
	ORESULT UseSkill(const AI_Character* pAI, ObjID_t idSkill, ObjID_t idTarget, FLOAT fTargetX, FLOAT fTargetZ, INT iHurtDelayTime, FLOAT fDir = -1.f, GUID_t guidTarget=INVALID_ID);
...
struct StateList
{
	...
	~StateList() {
		SAFE_DELETE_ARRAY(m_pStateList);
	}
