...
__ENTER_FUNCTION
	...
	if (OR_OK == oResult) {
		...
		{
			//如果技能合法有效，并且不是自动连续释放技能则压入技能使用队列
...
VOID AI_Human::AI_Logic_Combat( UINT uTime )
{
	...
	if (INVALID_ID != BagIndex)
	{
		...
		//如果当前技能和上一个技能属于同一个冷却组，并且冷却时间未结束则返回
...
__ENTER_FUNCTION
	...
	// 当前的死亡惩罚和概率没有关系只有一种死亡惩罚，就是装备掉持久，并且死亡惩罚只在被怪物打死的时候才有

	// 如果玩家小于等于十级，则耐久度不降低
	if ( pHuman->GetLevel() > 10 )
	{
		// 装备耐久度损失函数, 还有很多现成的惩罚函数，和这个函数在同一位置声明
...
__ENTER_FUNCTION
	...
	AssertEx(pEnemy,"Obj_Character到Obj_Human指针转换出错,AI_Human");
	
	// 查看关系表中是否已经有该玩家
	RELATION_TYPE RelationType;
	const _OWN_RELATION* Rel = pMyRelation->GetRelationInfo( pEnemy->GetName() );
	
	// 如果没有找到，就是和杀人者之前没有过社交关系
	if ( NULL == Rel )
	{
	...
	switch( RelationType )
	{
	case RELATION_TYPE_ENEMY:	// 如果已经是仇人，则不进行操作
		{
			return;
		}
	break;
