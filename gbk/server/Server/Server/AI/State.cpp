...
ORESULT State::UseSkill(const AI_Character* pAI, ObjID_t idSkill, ObjID_t idTarget, FLOAT fTargetX, FLOAT fTargetZ, INT iHurtDelayTime, FLOAT fDir/* = -1.f*/, GUID_t guidTarget)
{
	__ENTER_FUNCTION
		if (!pAI) {
			return OR_ERROR;
		...
		if (pCharacter && pCharacter->CanUseSkillNow(idSkill)) {
			...
			pos.m_fX = fTargetX;
			pos.m_fZ = fTargetZ;
			return pCharacter->Do_UseSkill(idSkill, idTarget, &pos, iHurtDelayTime, fDir, guidTarget);
		}
...
ORESULT State::Obj_UseSkill(const AI_Character* pAI, ObjID_t idSkill, ObjID_t idTarget, FLOAT fTargetX, FLOAT fTargetZ, INT iHurtDelayTime, FLOAT fDir, GUID_t guidTarget)
{
	__ENTER_FUNCTION
	if (!pAI) {
		return OR_ERROR;
