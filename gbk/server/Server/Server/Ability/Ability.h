...
protected:
	...
	INT							m_LevelUpConfigIndex;	// 升级配置的信息在AbilityLevelUpConfigManager中的索引
	...
	BOOL						m_BindFlag;             //高级合成中的物品绑定状态
	INT							m_BagIndex;             //高级合成中生成物品的包位置
...
struct _DBC_PRESCRIPTION_DATA
{
	...
	INT							m_ScriptID;
	UINT						m_ReplicaID;
	INT							m_nSpecStuffSubClass;	// 特殊材料的物品子类（From CommonItem.txt），若填写则必须
	...
	enum
	{
		...
		TB_Attr_RESERVED,				// 使用该配方需要耗损的门派贡献度值
		...
		TB_CompoundDesc,				// 详细描述
		TB_CompoundQuality,				// 合成物质量
		TB_SubClass,					// 特殊材料的物品子类（From CommonItem.txt），若填写则必须
	};
};
