...
enum ITEM_OPERATOR_ERROR//此枚举结构内的数据必须都为负数
{
	...
	ITEMOE_DESTOPERATOR_FAIL	= -11,	//目标容器物品不能设置
	ITEMOE_DESTOPUSE_FAIL		= -12,  //目标容器失效
	ITEMOE_SOUROPUSE_FAIL		= -13,  //源容器失效
	ITEMOE_DESTOPERATOR_INVALID = -14,	//目标位置无效
	ITEMOE_SOUROPERATOR_INVALID = -15,	//源位置无效
};
...
public :
	...
	BOOL  						SetSmartnessPer(ItemContainer*	pContainer,INT nIndex,INT conper);					//敏捷资质	
	BOOL   						SetMindPer(ItemContainer*	pContainer,INT nIndex,INT dexper);						//智力资质
	BOOL   						SetConstitutionPer(ItemContainer*	pContainer,INT nIndex,INT intper);				//体质资质
	...
	BOOL  						SetSavvy(ItemContainer*	pContainer,INT nIndex,INT iSavvy);							//宠物悟性
...
public:
	...
	//在容器pDestContainer生成一件物品，此物品具有pTemplateItem逻辑数据及属性
	//物品生成到pDestContainer容器内随机找到
	//的一个空位置上
	//uConIndex 最后物品放置的位置
	//注意：此操作产生一个具有唯一(GUID) 的pTemplateItem物品
	BOOL				CreateItem( ITEM_LOG_PARAM* pLogParam,
									UINT uQuality,
									ItemContainer* pDestContainer,
									UINT&	uConIndex,
