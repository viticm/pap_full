...
__ENTER_FUNCTION
	...
	if( pMonsterFile==NULL || g_Command_AlwaysLoadMonsterIni==TRUE )
	{
		...
		pMonsterFile->m_pInit = new _OBJ_MONSTER_INIT[iMonsterCount] ;
		Assert( pMonsterFile->m_pInit ) ;
		if( pMonsterFile->m_pInit==NULL )
			return FALSE ;
...
//			init.m_idScript = (ScriptID_t)atoi( szReadText );
			...
			NPC_QUEST* pQuest = g_NpcQuestTbl.Get(init.m_uDataID);
			if(pQuest!=NULL)
			{
	...
	else
	{
		...
		memcpy( pMonsterFile->m_pInitUsing, pMonsterFile->m_pInit, sizeof(_OBJ_MONSTER_INIT)*pMonsterFile->m_Count ) ;
	}

	//生成怪物
	for( i=0; i<pMonsterFile->m_CountUsing; i++ )
	{
		pExtAttr = g_MonsterAttrExTbl.GetExtAttr(pMonsterFile->m_pInitUsing[i].m_uDataID);
		if ( pExtAttr == NULL )
		{
			continue;
		}
...
__ENTER_FUNCTION
	...
	g_pLog->FastSaveLog( LOG_FILE_1, "RemoveAllMonster Finish curMonster count =%d", GetCount() );
	return TRUE ;
...
#define MAX_REMOVE_SIZE 1024
	...
	for ( i = 0; i < (INT)m_uCount; i++ )
	{
		if ( m_papObjs[i] != NULL )
		{
			...
			_MY_CATCH
			{
				//Log::SaveLog( SERVER_ERRORFILE, "MonsterManager::HeartBeat error, m_uCount = %d, i = %d ", 
				//	m_uCount, i );
				SaveCodeLog( ) ;
			}
		}
...
ObjID_t	MonsterManager::FindMonsterByDataID( UINT uDataID )
{
__ENTER_FUNCTION
