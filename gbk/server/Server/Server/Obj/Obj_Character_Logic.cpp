...
__ENTER_FUNCTION
	...
	else // 使用技能成功后，发送技能属性给客户端，用于显示技能名称		20100602 BLL
	{
		if ( this->GetObjType() == OBJ_TYPE_HUMAN ) // 如果是玩家使用技能，则给玩家自己发
		{
			GCCharUseSkill msg;
		...
		else if ( this->GetObjType() == OBJ_TYPE_MONSTER ) // 如果是怪物使用技能，则给目标玩家发
		{
			GCCharUseSkill msg;
...
BOOL Obj_Character::Obj_Logic_Move( UINT uTime )
{
	...
	Assert( fSpeed > 0.1 && fSpeed < 20.0 ); // 做个验证很重要，fancy
