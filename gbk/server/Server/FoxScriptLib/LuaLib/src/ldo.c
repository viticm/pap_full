 /*
** $Id: ldo.c,v 1.2 2006/06/17 09:16:22 zengpengxiang Exp $
...
// Argumant		: const char *buff
// Argumant		: size_t size
// Argumant		: const char *name
// Comments		:
// Author		: Romandou
*****************************************************************************/
LUA_API int lua_compilebuffer(lua_State *L, const char *buff, size_t size, const char *name)
{
	int status = parse_buffer(L,buff,size,name);
	return status;
}
...
LUA_API int lua_compilefile (lua_State *L, const char *filename) {
	int status = parse_file(L, filename);
	return status;
}
