...
__ENTER_FUNCTION
	...
	Assert(pPlayer);
	Assert(pPacket->GetType() == LBLNewPrize::BL_NEW_PRIZE);
	...
	if(pPlayer->IsBillingPlayer())
	{
		...
		if(/*BNPR_SUCCESS*/0x00 == pPacket->GetPrizeResult())
		{
			for(INT i = 0; i < pPacket->GetPrizeNum(); ++i)
			{
				...
				g_pLog->FastSaveLog(LOG_FILE_0,"LBLNewPrize...GetPrize Acc=%s price=%s,num=%d",
		...
		else if(/*BNPR_ALREADY_GET*/0x02 == pPacket->GetPrizeResult())
		{
			for(INT i = 0; i < pPacket->GetPrizeNum(); ++i)
			{
				//已经领过的奖品就没啥意思了
				//Msg.AddPrize(pPacket->GetPrize(i));
				g_pLog->FastSaveLog(LOG_FILE_0,"LBLNewPrize...AlreadyGetPrize Acc=%s price=%s,num=%d Drop",
					pPacket->GetAccount(), pPacket->GetPrize(i)->m_PrizeString, pPacket->GetPrize(i)->m_PrizeNum);
			}
		}
		...
		g_pLog->FastSaveLog(LOG_FILE_0,"LBLNewPrize::Execute()...OK Acc=%s PUID=%d,Result = %d",
