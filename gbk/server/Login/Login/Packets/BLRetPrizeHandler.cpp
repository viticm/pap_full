	...
	if(pPlayer->IsBillingPlayer())
	{
		...
		if(/*BPR_SUCCESS*/0x00 == pPacket->GetPrizeResult())
		{
			for(INT i = 0; i < pPacket->GetPrizeNum(); ++i)
			{
				...
				g_pLog->FastSaveLog(LOG_FILE_0,"BLRetPrize...GetPrize Acc=%s price=%s,num=%d",
		...
		else if(/*BPR_ALREADY_GET*/0x02 == pPacket->GetPrizeResult())
		{
			for(INT i = 0; i < pPacket->GetPrizeNum(); ++i)
			{
				//已经领过的奖品就没啥意思了
				//Msg.AddPrize(pPacket->GetPrize(i));
				g_pLog->FastSaveLog(LOG_FILE_0,"BLRetPrize...AlreadyGetPrize Acc=%s price=%s,num=%d Drop",
					pPacket->GetAccount(), pPacket->GetPrize(i)->m_PrizeString, pPacket->GetPrize(i)->m_PrizeNum);
			}
		}
		...
		g_pLog->FastSaveLog(LOG_FILE_0,"BLRetPrize::Execute()...OK Acc=%s PrizeSerial=%s PUID=%d,Result = %d",
			pPacket->GetAccount(),pPacket->GetPrizeSerial(),pPacket->GetPacketIndex(),pPacket->GetPrizeResult());
	}
