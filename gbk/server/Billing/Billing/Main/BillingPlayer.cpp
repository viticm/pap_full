...
BOOL BillingPlayer::ProcessCommand( BOOL Option )
{
	...
	_MY_TRY
	{
		...
		for( ;; )
		{
			if( !((SocketBillingInputStream*)m_pSocketInputStream)->Peek(&header[0], BILLING_PACKET_HEADER_SIZE+sizeof(SHORT)) )
			{//数据不能填充消息头
				break ;
			}
			...
			g_pLog->FastSaveLog(LOG_FILE_0, "PacketID = %d", PacketID);
			...
			_MY_TRY
			{
				...
				if( ((SocketBillingInputStream*)m_pSocketInputStream)->Length()<BILLING_PACKET_HEADER_SIZE+BILLING_PACKET_TAIL_SIZE+PacketSize-1)
				{//消息没有接收全
